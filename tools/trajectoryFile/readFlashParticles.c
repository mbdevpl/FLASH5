...
int readFlashParticles(hid_t file_identifier, double* localParticles, 
  ...
  hid_t    dataspace, dataset, memspace;
	...
	if (status < 0){
	    ...
	    fprintf(stdout,"******Error Reading Particle data!********\n");
	    fprintf(stdout,"****** npart_props = %d ********\n", npart_props);
	    fprintf(stdout,"****** offset: %d | localnp: %d | sum: %d ********\n",
		    offset, localnp, offset+localnp);
            fprintf(stderr,"******Error Reading Particle data!********\n");
            fprintf(stderr,"****** npart_props = %d ********\n", npart_props);
            fprintf(stderr,"****** offset: %d | localnp: %d | sum: %d ********\n",
                    offset, localnp, offset+localnp);
...
int flashFileOpen(char* filename, hid_t* file_identifier, int create, 
		  MPI_Comm comm){
    ...
    ierr = H5Pset_fapl_mpio(acc_template, comm, fileInfoTemplate);
...
int flashFileOpenReadOnly(char* filename, hid_t* file_identifier){
    ...
    *file_identifier = H5Fopen(filename, H5F_ACC_RDONLY, acc_template);
...
int getNumberOfParticles(hid_t *fidIn, int *numParticles, int *numPartProps){
  ...
  hsize_t dimens[2], maxdimens[2];

  dataset = H5Dopen(*fidIn, "tracer particles");
  dataspace = H5Dget_space(dataset);

  H5Sget_simple_extent_dims(dataspace, dimens, maxdimens);
  
  *numParticles = (int)dimens[0];
  *numPartProps = (int)dimens[1];
...
int getMetadata(hid_t *fidIn, int* numParticles, int* tagIndex, int* numPartProps){

  getNumberOfParticles(fidIn, numParticles, numPartProps);
