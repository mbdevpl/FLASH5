...
; This function does a cell-wise division of two amr objects of
; identical structure and returns a new amr object. If amr2 is a pure
; number rather than an amr object, the routine divides every cell of
; amr1 by amr2.
...
for n=0, amr1.maxlevel do begin
	...
	for m=0, amr1.levels[n].nfab-1 do begin
		...
		if isAMR2 then begin
		   ...
		   ; form the quotient
		   dataqot = *(*amr1.levels[n].fabptr)[m].dataptr / $
			     *(*amr2.levels[n].fabptr)[m].dataptr
		endif else begin
		   dataqot = *(*amr1.levels[n].fabptr)[m].dataptr / amr2
		endelse

		; point to the result
		(*amr_new.levels[n].fabptr)[m].dataptr = ptr_new(dataqot)
