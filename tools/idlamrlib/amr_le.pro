...
if isAMR2 then begin
	if (amr1.name ne amr2.name) then amr_new.name = amr1.name + ' le ' $
		+ amr2.name
	amr_new.componentname = '(' + amr1.componentname + ' le ' $
		+ amr2.componentname + ')'
...
for n=0, amr1.maxlevel do begin
	...
	for m=0, amr1.levels[n].nfab-1 do begin
		...
		; do consistency checks on amr1 and amr2 in this fab
		   ...
		   ; form the le
		   datale = *(*amr1.levels[n].fabptr)[m].dataptr le $
			    *(*amr2.levels[n].fabptr)[m].dataptr
		endif else begin
		   datale = *(*amr1.levels[n].fabptr)[m].dataptr le amr2
		endelse

		; point to the result
		(*amr_new.levels[n].fabptr)[m].dataptr = ptr_new(datale)
