function min_amr, amr, max, maxidx=maxidx, minidx=minidx
...
; this routine finds the minimum value in an amr object. It works just
...
for n=0, amr.maxlevel do begin
	for m=0, amr.levels[n].nfab-1 do begin
		...
		; get min
		localmin = min(*(*amr.levels[n].fabptr)[m].dataptr, idx)
		...
		if n_params() eq 2 then begin
			localmax = $
			   max(*(*amr.levels[n].fabptr)[m].dataptr, idx)
