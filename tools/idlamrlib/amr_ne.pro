...
; This function does a cell-wise addition of two amr objects of
; identical structure and returns a new amr object. If amr2 is a pure
; number rather than an amr object, it is simply added to every cell
; of amr1.
...
if isAMR2 then begin
	if (amr1.name ne amr2.name) then amr_new.name = amr1.name + ' ne ' $
		+ amr2.name
	amr_new.componentname = '(' + amr1.componentname + ' ne ' $
		+ amr2.componentname + ')'
	...
	amr_new.componentname = '(' + strtrim(string(amr2),2) + $
		' ne ' + amr1.componentname + ')'
endelse
...
for n=0, amr1.maxlevel do begin
	...
	for m=0, amr1.levels[n].nfab-1 do begin
		...
		; do consistency checks on amr1 and amr2 in this fab
		   ...
		   ; form the ne
		   datane = *(*amr1.levels[n].fabptr)[m].dataptr ne $
			    *(*amr2.levels[n].fabptr)[m].dataptr
		endif else begin
		   datane = *(*amr1.levels[n].fabptr)[m].dataptr ne amr2
		endelse

		; point to the result
		(*amr_new.levels[n].fabptr)[m].dataptr = ptr_new(datane)
