function amr_multiply, amr1, amr2
...
; This function does a cell-wise multiplication of two amr objects of
; identical structure and returns a new amr object. Alternately, if amr2
; is a pure number rather than an amr object, the routine multiplies
; the amr object by the pure number.
...
for n=0, amr1.maxlevel do begin
	...
	for m=0, amr1.levels[n].nfab-1 do begin
		...
		if isAMR2 then begin
		   ...
		   ; form the producet
		   dataprod = *(*amr1.levels[n].fabptr)[m].dataptr * $
			      *(*amr2.levels[n].fabptr)[m].dataptr
		endif else begin
		   dataprod = *(*amr1.levels[n].fabptr)[m].dataptr * amr2
		endelse

		; point to the result
		(*amr_new.levels[n].fabptr)[m].dataptr = ptr_new(dataprod)
