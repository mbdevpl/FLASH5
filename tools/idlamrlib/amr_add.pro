...
for n=0, amr1.maxlevel do begin
	...
	for m=0, amr1.levels[n].nfab-1 do begin
		...
		; do consistency checks on amr1 and amr2 in this fab
		   ...
		   ; form the sum
		   datasum = *(*amr1.levels[n].fabptr)[m].dataptr + $
			     *(*amr2.levels[n].fabptr)[m].dataptr
		endif else begin
		   datasum = *(*amr1.levels[n].fabptr)[m].dataptr + amr2
		endelse

		; point to the result
		(*amr_new.levels[n].fabptr)[m].dataptr = ptr_new(datasum)
