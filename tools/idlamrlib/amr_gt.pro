...
if isAMR2 then begin
	if (amr1.name ne amr2.name) then amr_new.name = amr1.name + ' gt ' $
		+ amr2.name
	amr_new.componentname = '(' + amr1.componentname + ' gt ' $
		+ amr2.componentname + ')'
...
for n=0, amr1.maxlevel do begin
	...
	for m=0, amr1.levels[n].nfab-1 do begin
		...
		; do consistency checks on amr1 and amr2 in this fab
		   ...
		   ; form the gt
		   datagt = *(*amr1.levels[n].fabptr)[m].dataptr gt $
			    *(*amr2.levels[n].fabptr)[m].dataptr
		endif else begin
		   datagt = *(*amr1.levels[n].fabptr)[m].dataptr gt amr2
		endelse
