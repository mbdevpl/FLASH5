...
/*meant to be overridden by a more advanced decomposition later.
 *for now decompose an even number of blocks
*/
/*int PFOCU_DecomposeDomain(FR_File A, decomp_1d * decomp, int mype, int nprocs){

  int s, e;  
  MPE_Decomp1d(A.nblocks, nprocs, mype, &s, &e);
  decomp->start = s;
  decomp->end = e;
  decomp->total = e - s + 1;  
  return 0;
...
FR_File *FR_open_HDF5(char *filename){
  ...
  handle = H5Fopen(out->filename, H5F_ACC_RDONLY, plist_id);
