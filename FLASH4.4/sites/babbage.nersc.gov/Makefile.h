# FLASH makefile definitions for Babbage, a Intel MIC testbed at NERSC.
#
...
#VECTORIZE = -vec-report3
VECTORIZE = -no-vec -no-simd -vec-report3

#FFLAGS_ALIGN = -align array64byte
FFLAGS_ALIGN =

FFLAGS_MIC   = -mmic ${VECTORIZE} ${FFLAGS_ALIGN}
CFLAGS_MIC   = -mmic ${VECTORIZE}
LFLAGS_MIC   = ${FFLAGS_MIC}


# Suppress the following compiler warning message:
# icc: command line warning #10120: overriding '-O2' with '-O3'

FFLAGS_OPT   = -c -O3 ${FFLAGS_MIC} -g -r8 -i4 -real_size 64 -diag-disable 10120
FFLAGS_DEBUG = -c -O0 ${FFLAGS_MIC} -g -r8 -i4 -check bounds -check format \
-check output_conversion -warn all -warn error -real_size 64 -check uninit \
-traceback -fp-stack-check -diag-disable 10120 -check pointers
FFLAGS_TEST  = ${FFLAGS_OPT} -fp-model precise

CFLAGS_OPT   = -c -O3 ${CFLAGS_MIC} -g -D_LARGEFILE64_SOURCE -D_FORTIFY_SOURCE=2 \
-diag-disable 10120
CFLAGS_DEBUG = -c -O0 ${CFLAGS_MIC} -g -traceback -debug all -debug extended \
...
LFLAGS_OPT   = -O3 ${LFLAGS_MIC} -diag-disable 10120 -o
LFLAGS_DEBUG = ${LFLAGS_MIC} -diag-disable 10120 -o
