...
#include "Flash.h"
  ...
  real :: extraHydroInfoMin, tp_extraHydroInfoMin
  real :: extraHydroInfoApp, tp_extraHydroInfoApp
  real :: dtNewComputed
  ...
  !$omp extraHydroInfoMin,extraHydroInfoApp,dr_useSTS,dr_dtSTS) &
  ...
  !$omp blkLimits,blkLimitsGC,del,tp_dtLocal,tp_lminloc,&
  !$omp extraHydroInfo,tp_extraHydroInfoMin,tp_extraHydroInfoApp,solnData)

  tp_dtLocal = dtLocal
  tp_lminloc = lminloc
  tp_extraHydroInfoMin = extraHydroInfoMin
  tp_extraHydroInfoApp = extraHydroInfoApp
  ...
  do i = 1, numLeafBlocks
     ...
     if (extraHydroInfo .ne. 0.) then
        if (extraHydroInfo <= tp_extraHydroInfoMin) then
           tp_extraHydroInfoMin = extraHydroInfo
        endif
        if (tp_lminloc(4,HYDRO) == blockList(i)) then
           tp_extraHydroInfoApp = extraHydroInfo
        endif
     else !if extraHydroInfo == 0.
        tp_extraHydroInfoMin = 0.0
        tp_extraHydroInfoApp = 0.0
     endif
...
!!$                           dtLocal(1,COSMO), lminloc(:,COSMO) )
  ...
  !$omp atomic
  extraHydroInfoMin = min(extraHydroInfoMin,tp_extraHydroInfoMin)

  !$omp critical (Reduce_dt_info)
  if (tp_dtLocal(1,HYDRO) < dtLocal(1,HYDRO)) then
     extraHydroInfoApp = tp_extraHydroInfoApp
  end if
  do i = 1, nUnits
