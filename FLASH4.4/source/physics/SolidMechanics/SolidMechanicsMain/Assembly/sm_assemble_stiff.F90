...
subroutine sm_assemble_stiff(ibd, iopt)
  ...
  use sm_assemble_interface, only: sm_assemble_stiff_3DFlexible, &
                                   sm_assemble_stiff_rigid, sm_assemble_stiff_rbc
  ...
  ! Load and allocate based on BodyType
   ...
   case( BODYTYPE_RIGID ) 
      call sm_assemble_stiff_rigid(ibd,flag)

   case( BODYTYPE_2DFLEXIBLE )
      call Driver_abortFlash('BodyType not yet implemented')

   case( BODYTYPE_3DFLEXIBLE )
      call sm_assemble_stiff_3DFlexible(ibd,flag)

   case( BODYTYPE_RBC )
      call sm_assemble_stiff_RBC(ibd,flag)

   case default
      call Driver_abortFlash("BodyType not yet implemented <sm_assemble_stiff.F90>")
