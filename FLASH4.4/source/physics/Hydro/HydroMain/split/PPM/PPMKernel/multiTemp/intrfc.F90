!!****if* source/physics/Hydro/HydroMain/split/PPM/PPMKernel/multiTemp/intrfc
!!
!! NAME
!! 
!!  subroutine intrfc
...
!!              real(INOUT) :: gamel(numCells,0:HYDRO_NUM_GAME_COMPONENTS),
!!              real(INOUT) :: gamer(numCells,0:HYDRO_NUM_GAME_COMPONENTS),
...
!! ----LOCAL ----------------
  real,dimension(numCells) :: rhog, rhogl, rhogr, drhog, rg6,&
                              ...
                              flatn, flatn1
  real,dimension(numCells,hy_numXn) :: xnLoc
...
! each zone.  The polynomial used is Eq. 1.6 in Colella & Woodward. 
  ...
  xnLoc(:,:) = xn(:,:)
#if (SELE_IND > 0)
  if (hy_dbgReconstConsvSele) xnLoc(:,SELE_IND) = xn(:,SELE_IND) * rho(:)
#endif
...
! each zone
  ...
  if (hy_charLimiting) then
     ...
     do n = 1, hy_numXn
        ...
        call interp(numIntCells, numCells,xnl(1,n), xnLoc(1,n), xnr(1,n),  &
                    coeff1, coeff2, coeff3, coeff4, coeff5)
     
        if (hy_useSteepening .and. n <= NSPECIES) & 
             call detect(numIntCells, numCells,xnl(1,n), xnLoc(1,n), xnr(1,n), hy_smallx,  &
                         rho, p, game, dx, x)
#ifdef SELE_MSCALAR
        if (hy_useSteepening .and. SPECIES_BEGIN-1+n == SELE_MSCALAR) & 
             call detect(numIntCells, numCells,xnl(1,n), xnLoc(1,n), xnr(1,n), hy_smallx,  &
                         rho, p, game, dx, x)
#endif
     end do
  ...
  do n = 0, HYDRO_NUM_GAME_COMPONENTS
     call interp(numIntCells, numCells,gamel(1,n), game(1,n), gamer(1,n),&
          coeff1,coeff2,coeff3,coeff4,coeff5)
  end do
  ...
  do n = 0, HYDRO_NUM_EINT_COMPONENTS
     call interp(numIntCells, numCells,eintl(1,n), eint(1,n), eintr(1,n),&
          coeff1,coeff2,coeff3,coeff4,coeff5)
  end do
  do n = 0, HYDRO_NUM_E_COMPONENTS
     call interp(numIntCells, numCells,etotl(1,n), etot(1,n), etotr(1,n),&
          coeff1,coeff2,coeff3,coeff4,coeff5)
  end do
  do n = 1, HYDRO_NUM_E_COMPONENTS
     call interp(numIntCells, numCells,pFracl(1,n), pFrac(1,n), pFracr(1,n),&
          coeff1,coeff2,coeff3,coeff4,coeff5)
  end do
  ...
  do i = 4, numIntCells5
     ...
     gamel(i,0:) = flatn(i) * game(i,0:) + flatn1(i) * gamel(i,0:)
     gamer(i,0:) = flatn(i) * game(i,0:) + flatn1(i) * gamer(i,0:)
     
     gamcl(i) = flatn(i) * gamc(i) + flatn1(i) * gamcl(i)
     gamcr(i) = flatn(i) * gamc(i) + flatn1(i) * gamcr(i)

     eintl(i,0:) = flatn(i) * eint(i,0:) + flatn1(i) * eintl(i,0:)
     eintr(i,0:) = flatn(i) * eint(i,0:) + flatn1(i) * eintr(i,0:)

     etotl(i,0:) = flatn(i) * etot(i,0:) + flatn1(i) * etotl(i,0:)
     etotr(i,0:) = flatn(i) * etot(i,0:) + flatn1(i) * etotr(i,0:)

     pFracl(i,1:) = flatn(i) * pFrac(i,1:) + flatn1(i) * pFracl(i,1:)
     pFracr(i,1:) = flatn(i) * pFrac(i,1:) + flatn1(i) * pFracr(i,1:)
  end do
    
  do n = 1, hy_numXn
     do  i = 4, numIntCells5
        xnl(i,n) = flatn(i) * xnLoc(i,n) + flatn1(i) * xnl(i,n)
        xnr(i,n) = flatn(i) * xnLoc(i,n) + flatn1(i) * xnr(i,n)
     end do
  end do
  ...
  do n = 0, HYDRO_NUM_EINT_COMPONENTS
     call monot(numIntCells, numCells,eintl(1,n), eint(1,n), eintr(1,n), hy_deint(1,n), hy_eint6(1,n))
  end do
  do n = 0, HYDRO_NUM_E_COMPONENTS
     call monot(numIntCells, numCells,etotl(1,n), etot(1,n), etotr(1,n), hy_detot(1,n), hy_etot6(1,n))
  end do
  do n = 1, HYDRO_NUM_E_COMPONENTS
     call monot(numIntCells, numCells,pFracl(1,n), pFrac(1,n), pFracr(1,n), hy_dpFrac(1,n), hy_pFrac6(1,n))
  end do
  do n = 0, HYDRO_NUM_GAME_COMPONENTS
     call monot(numIntCells, numCells,gamel(1,n), game(1,n), gamer(1,n), hy_dgame(1,n), hy_game6(1,n))
  end do

  call monot(numIntCells, numCells,hy_gravl, grav, hy_gravr, hy_dgrav, hy_grav6)
  
  do n = 1, hy_numXn
!     call hy_nomonot(numIntCells, numCells,xnl(1,n),xnLoc(1,n),xnr(1,n),hy_dxn(1,n), hy_xn6(1,n))
     call monot(numIntCells, numCells,xnl(1,n),xnLoc(1,n),xnr(1,n),hy_dxn(1,n), hy_xn6(1,n))
  end do
...
! if we are doing the CMA flattening of the abundances, do it now
  if (hy_useCmaFlattening) &
       call cma_flatten(numIntCells, numCells, guard,xnLoc, xnl, xnr, hy_dxn, hy_xn6)
