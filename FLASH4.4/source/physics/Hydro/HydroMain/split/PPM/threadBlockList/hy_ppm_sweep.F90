!!****if* source/physics/Hydro/HydroMain/split/PPM/threadBlockList/hy_ppm_sweep
!!
!! NAME
!!
!!  hy_ppm_sweep
...
#ifdef FIXEDBLOCKSIZE
  !CD - I changed numCells to a regular integer so that I can use the same
  !OpenMP declaration in FBS and NOFBS mode.  numCells was a Fortran
  !Parameters in FBS mode, but a parameter is a C-Preprocessor define and
  !so cannot appear in firstprivate clause.
  numCells = MAXCELLS
#endif
...
!!------------------------------------------------------------------
  ...
  !$omp default(none) &
  !$omp firstPrivate(updateMode,numCells) &
  !$omp private(blk,del,bcs,blkLimits,blkLimitsGC,isizeGC,jsizeGC,ksizeGC,&
  !$omp isize,jsize,ksize,tempFlx,tempGrav1d_o,tempGrav1d,tempDtDx,tempFict,&
  !$omp tempArea,tempAreaLeft,shock,primaryCoord,primaryLeftCoord,&
  !$omp jCoord,kCoord,sDetectLayers,&
  !$omp primaryRghtCoord,primaryDx,secondCoord,thirdCoord,k,j,i,eosGcLayers,&
  !$omp solnData,size,radialCoord,ugrid,isGC,numGuard,igeom,startingPos) &
  !$omp shared(hy_meshMe,hy_meshNumProcs,blockCount,timeEndAdv,dt,dtOld,sweepDir,&
  !$omp blockList,gCell,level,doFluxCorrect,idir,hy_eosMode,hy_irenorm,&
  !$omp hy_updateHydroFluxes,hy_useDiffuse,hy_dirgeom,hy_cvisc,&
  !$omp transverseEosLayers,&
  !$omp hy_hybridriemann,hy_specialfluxvars)
...
! Do this part only if refining and flux correcting
  ...
  if(doFluxCorrect) then
     ...
     !CD - Grid_conserveFluxes is not thread safe so only call with the master
     !thread.  We also depend on the implicit barrier at the end of the previous
     !parallel do...  don't be tempted to put nowait clause there!!!
     !$omp master
     call Grid_conserveFluxes(sweepDir, level)
     !$omp end master

     !$omp barrier
     !CD - Block all threads until the master thread has conserved fluxes.
