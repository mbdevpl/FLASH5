!!****if* source/physics/Hydro/HydroMain/split/PPM/chomboCompatible/hy_ppm_sweep
!!
!! NAME
!!
!!  hy_ppm_sweep
...
subroutine hy_ppm_sweep ( blockCount, blockList, &
  ...
  use Grid_interface, ONLY : Grid_fillGuardCells, Grid_getDeltas, &
    ...
    Grid_limitAbundance, Grid_primitiveToConserve, Grid_conserveToPrimitive, &
  ...
  use hy_ppm_chombo_interface, ONLY : hy_ppm_chomboLikeUpdateSoln, hy_ppm_flux_conserve
  use IO_interface, ONLY: IO_writeCheckpoint
  use RuntimeParameters_interface, ONLY : RuntimeParameters_get
  use Driver_interface, ONLY : Driver_abortFlash
#ifdef FLASH_GRID_CHOMBO
  use chombo_f_c_interface, ONLY : ch_zero_flux_data
#endif
  implicit none
  ...
  real, pointer :: primaryCoord(:) , &
                   ...
                   thirdCoord(:)
  integer :: i,j,k,blk
  integer, dimension(2,MDIM) :: lim,blkLimits,blkLimitsGC,bcs
...
#endif
  logical :: chomboLikeUpdateSoln, excludeGradPresFromFlux
  call RuntimeParameters_get("chomboLikeUpdateSoln", chomboLikeUpdateSoln)   
  call RuntimeParameters_get("excludeGradPresFromFlux", excludeGradPresFromFlux)


#ifdef FLASH_GRID_CHOMBO
  if (.not.chomboLikeUpdateSoln) then
     call Driver_abortFlash("chomboLikeUpdateSoln must be .true. for Chombo")
  end if
#endif
...
!  end if
  ...
  ! The Chombo implementation of Grid_fillGuardCells probably just ignores
  ! the idir,minLayers,selectBlockType as well as the mask-related arguments,
...
!!--------- FLUX CONSERVATION
     if (doFluxCorrect) then
        ...
        if(.not.excludeGradPresFromFlux) then
           lim = blkLimits
           if (sweepDir==SWEEP_X) then
              lim(HIGH,IAXIS) = lim(HIGH,IAXIS) + 1
           else if (sweepDir==SWEEP_Y) then
              lim(HIGH,JAXIS) = lim(HIGH,JAXIS) + 1
           else if (sweepDir==SWEEP_Z) then
              lim(HIGH,KAXIS) = lim(HIGH,KAXIS) + 1
           end if

           !Only internal values of tempAreaLeft contain initialized values.
           do kk = lim(LOW,KAXIS), lim(HIGH,KAXIS)
              do jj = lim(LOW,JAXIS), lim(HIGH,JAXIS)
                 do ii = lim(LOW,IAXIS), lim(HIGH,IAXIS)
                    tempFlx(U_FLUX,ii,jj,kk) = tempFlx(U_FLUX,ii,jj,kk) + &
                         tempAreaLeft(ii,jj,kk) * tempFlx(P_FLUX,ii,jj,kk)
                 end do
              end do
           end do
        end if
        ...
        !Does not make sense for Chombo version + it seems that the arrays
        !inside hy_ppm_putTemporaryData are not allocated in NOFBS mode.
...
! Do this part only if refining and flux correcting
  ...
  if(doFluxCorrect) then
     ...
     if (chomboLikeUpdateSoln.eqv..true.) then
        !The subroutines Grid_primitiveToConserve and Grid_conserveToPrimitive
        !perform operations on all internal cells.  This is redundant work
        !because, in theory, only the edge internal cells are needed.
        !Updating all cells may be helpful in our experiments with Chombo
        !because it ensures that all block data is in conservative form.
        call Grid_primitiveToConserve(blockList, blockCount, .true.)
     end if
     ...
     !Execute this loop for Paramesh only.  In Chombo applications
     !Grid_conserveFluxes includes the solution update.
#ifdef FLASH_GRID_PARAMESH
     ...
     do blk = 1,blockCount
        ...
        call Timers_start("hy_ppm_updateSoln")
        if (chomboLikeUpdateSoln.eqv..true.) then
           call hy_ppm_chomboLikeUpdateSoln(updateMode, &
                sweepDir, dt,&
                blkLimits,blkLimitsGC,numCells, &
                tempArea, tempGrav1d_o, tempGrav1d, &
                tempDtDx, tempFict,   &
     ...
     if (chomboLikeUpdateSoln.eqv..true.) then
        call Grid_conserveToPrimitive(blockList, blockCount, .false., .true.)
        call hy_ppm_flux_conserve(POST_FLUX_CONSERVE, sweepDir, blockCount, blockList)
     end if
...
!!!        call Hydro_recalibrateEints(blkLimits, blockList(blk))
     ...
     end do !!END FINAL LOOP OVER BLOCKS

  end if


  !DEV CD: We think that there should be an explicit call to restrict
  !after each sweep.  We need to review this another time.
