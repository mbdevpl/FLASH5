!!****if* source/physics/Hydro/HydroMain/split/PPM/threadWithinBlock/hy_ppm_block
!!
!! NAME
!!
!!  hy_ppm_block
...
subroutine hy_ppm_block( hy_meshMe,blockID,sweepDir, dt, dtOld, &
  ...
  use Hydro_data, ONLY : hy_useGravity, hy_dirGeom,&
                         ...
                         hy_gravMass, hy_threadWithinBlock
...
#include "PPM.h"
  ...
  integer :: numIntCells
  integer :: numXn=NSPECIES+NMASS_SCALARS
  integer :: sp,istat
  ...
  !Shared variables.
  !Lines 1-3 are intent(IN) arguments which cannot be modified in
  ! this subroutine and solnData which is not modified in this subroutine.
  !Lines 4-5 are modified in this subroutine.
  ! Different threads modify different sections of these arrays except for
  ! hy_gravMass which is atomically updated.
  !Lines 6-7 are module level variables and numXn.  They are not
  ! modified in this subroutine.

  !Note that all other hy_ variables are threadprivate and so do not appear
  !in the parallel construct.

#ifdef __PGI
  !Avoid strict default(none) checks for PGI compiler.  This is a PGI
  !compiler bug because threadprivate variables should not appear in
  !the parallel construct.  The original error was
  !PGF90-S-0155-hy_pstor must appear in a SHARED or PRIVATE clause
  !PGF90-S-0155-hy_pstor must appear in a SHARED or PRIVATE clause
#define DEFAULT_CLAUSE_MACRO
#else
#define DEFAULT_CLAUSE_MACRO default(none)
#endif

  !$omp parallel if (hy_threadWithinBlock) DEFAULT_CLAUSE_MACRO &
  !$omp shared(hy_meshMe,blockID,sweepDir,dt,dtOld,numCells,numguard,blkLimitsGC,&
  !$omp blkLimits,bcs,shock,primaryCoord,primaryLeftCoord,primaryRghtCoord,&
  !$omp primaryDx,secondCoord,thirdCoord,radialCoord,ugrid,solnData,&
  !$omp tempArea,tempGrav1d_o,tempGrav1d,tempDtDx,tempFict,tempAreaLeft,&
  !$omp tempFlx,hy_gravMass,&
  !$omp hy_nriem,hy_usecellareasforfluxes,hy_usegravity,hy_dirgeom,&
  !$omp hy_updatehydrofluxes,numXn) &
  !$omp private(i1,i2,j1,j2,k1,k2,i,j,k,ii,ilo,ihi,jlo,jhi,klo,khi,&
  !$omp iloGc,ihiGc,jloGc,jhiGc,kloGc,khiGc,&
  !$omp xbot,xtop,ybot,ytop,ylft,yrgt,zlft,zrgt,&
  !$omp numIntCells,sp,istat,point,cellVolume,faceAreas,cellVolumes,&
  !$omp dtdx,areaLeft,area,cvol,grav,ngrav,fict,shock_multid,&
  !$omp rhoflx,uflx,pav,utflx,uttflx,eflx,eintflx,&
  !$omp u,ut,utt,rho,p,e,tmp,gamc,game,uttp,utbt,utlt,utrt,xn,xnflx,tmpGravMass)
...
!This array is shared so we must ensure no conflicts.  Implicit barrier at
!end of workshare directive.
!$omp workshare
  tempFlx(:,:,:,:) = 0.0
!$omp end workshare
...
!        call Timers_stop("hydro_1d")
           ...
           tempAreaLeft(ilo:ihi+1,j,k) = areaLeft(ilo:ihi+1)  !OPENMP - distinct (j,k) section in tempAreaLeft.
         if (hy_updateHydroFluxes) then
           tmpGravMass = 0.0
           do i = ilo, ihi
              tempDtDx(i,j,k)     = dtdx(i)  !OPENMP - distinct (j,k) section in tempDtDx.
              tempArea(i,j,k)     = area(i)  !OPENMP - distinct (j,k) section in tempArea.
              tempGrav1d_o(i,j,k) = grav(i)  !OPENMP - distinct (j,k) section in tempGrav1d_o.
              tempGrav1d(i,j,k)   = ngrav(i) !OPENMP - distinct (j,k) section in tempGrav1d.
              tempFict(i,j,k)     = fict(i)  !OPENMP - distinct (j,k) section in tempFict.
           ...
           !$omp atomic
           hy_gravMass(IAXIS) = hy_gravMass(IAXIS) + tmpGravMass   !OPENMP - accumulation.
           ...
           enddo  !OPENMP - distinct (j,k) section in tempFlx.
           
           do sp = 1, numXn
              do i = ilo, ihi+1
                 tempFlx(SPECIES_FLUX_BEGIN+sp-1,i,j,k) = xnflx(i,sp)  !OPENMP - distinct (j,k) section in tempFlx.
              end do
           enddo
         else
           do i = ilo, ihi
  ...
  !$omp end do nowait
#endif
!OPENMP - No need to synchronise here as we can only enter a single select case
!option per call.  We will synchronise at the end of the parallel region which is at the
!bottom of this subroutine.
...
!        call Timers_stop("hydro_1d")
         ...
         if (hy_updateHydroFluxes) then
           ...
           do j = jlo, jhi
              ...
              point(JAXIS) = j
              call Grid_getSingleCellVol(blockID,EXTERIOR,point,cellVolume)
              tmpGravMass = tmpGravMass + grav(j)*rho(j)*cellVolume
           enddo
