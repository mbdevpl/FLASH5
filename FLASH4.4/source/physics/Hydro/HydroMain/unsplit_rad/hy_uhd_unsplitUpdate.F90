!!****if* source/physics/Hydro/HydroMain/unsplit_rad/hy_uhd_unsplitUpdate
!!
!! NAME
!!
...
#else
                !! Fluxes at each local cell 
                FL(HY_DENS_FLUX:) = xflux(HY_DENS_FLUX:,i,  j,   k   )
                FR(HY_DENS_FLUX:) = xflux(HY_DENS_FLUX:,i+1,j,   k   )
                GL(HY_DENS_FLUX:) = yflux(HY_DENS_FLUX:,i,  j,   k   )*ky
                GR(HY_DENS_FLUX:) = yflux(HY_DENS_FLUX:,i,  j+ky,k   )*ky
                HL(HY_DENS_FLUX:) = zflux(HY_DENS_FLUX:,i,  j,   k   )*kz
                HR(HY_DENS_FLUX:) = zflux(HY_DENS_FLUX:,i,  j,   k+kz)*kz
#endif
...
!!$                flRght                  = FR(HY_P_FLUX)
                ...
                ! pmat = (game-1) (eele_flx + eion_flx)

#ifdef FLASH_UHD_3TRADFLAH
                lam3Rad = U(FLLM_VAR,i,j,k) * hy_lam3ScaleFactor

                Sphys(HY_XMOM) =  (FL(HY_PMAT_FLUX) - FR(HY_PMAT_FLUX) + &
                          lam3Rad*(FL(HY_PRAD_FLUX) - FR(HY_PRAD_FLUX))  &
                                   ) *(1.0-fP)/ dx
#else
                lam3Rad = 1.0
                Sphys(HY_XMOM)                  = (FL(HY_P_FLUX) - FR(HY_P_FLUX)) *(1.0-fP)/ dx
!!$                flLeft = min(flLeft,FL(HY_PMAT_FLUX)+lam3Rad*FL(HY_PRAD_FLUX))
!!$                flRght = min(flRght,FR(HY_PMAT_FLUX)+lam3Rad*FR(HY_PRAD_FLUX))
#endif

!                Sphys(HY_XMOM)                  = (flLeft - flRght) / dx
#ifdef DEBUG_FEB2015
                print*,i,FL(HY_P_FLUX),FR(HY_P_FLUX),FL(HY_PMAT_FLUX),FR(HY_PMAT_FLUX),&
                     FL(HY_PRAD_FLUX),FR(HY_PRAD_FLUX),lam3Rad
#endif
...
!!$#ifdef FLASH_UHD_3TRADFLAH
!!$                   ccPres =            FL(HY_PMAT_FLUX) + FR(HY_PMAT_FLUX) + &
!!$                                lam3Rad*(FL(HY_PRAD_FLUX) + FR(HY_PRAD_FLUX))
!!$                   if (NDIM>1) then
!!$                      ccPres = ccPres + GL(HY_PMAT_FLUX) + GR(HY_PMAT_FLUX) + &
!!$                                   lam3Rad*(GL(HY_PRAD_FLUX) + GR(HY_PRAD_FLUX))
!!$                   end if
!!$                   if (NDIM==3) then
!!$                      ccPres = ccPres + HL(HY_PMAT_FLUX) + HR(HY_PMAT_FLUX) + &
!!$                                   lam3Rad*(HL(HY_PRAD_FLUX) + HR(HY_PRAD_FLUX))
!!$                   end if
!!$                   tempPres = ccPres / (2.0*NDIM)
!!$#endif

#ifdef FLASH_UHD_3T
                   call updateInternalEnergy3T(IntEner,&
                         xflux(HY_P_FLUX:HY_PMAT_FLUX,i  ,j  ,k  ),&
                         xflux(HY_P_FLUX:HY_PMAT_FLUX,i+1,j  ,k  ),&
                         yflux(HY_P_FLUX:HY_PMAT_FLUX,i  ,j   ,k),&
                         yflux(HY_P_FLUX:HY_PMAT_FLUX,i  ,j+ky,k),&
                         zflux(HY_P_FLUX:HY_PMAT_FLUX,i  ,j   ,k),&
                         zflux(HY_P_FLUX:HY_PMAT_FLUX,i  ,j   ,k+kz),&
                         FL(HY_EINT_FLUX:HY_END_FLUX),&
                         FR(HY_EINT_FLUX:HY_END_FLUX),&
                         GL(HY_EINT_FLUX:HY_END_FLUX),&
                         GR(HY_EINT_FLUX:HY_END_FLUX),&
                         HL(HY_EINT_FLUX:HY_END_FLUX),&
                         HR(HY_EINT_FLUX:HY_END_FLUX),&
                         dxv,dyv,dz,dt,lam3Rad,Qohm)
#else
...
!!$                      tmomStar = U0(MOM_THT) 
  ...
  !! ==================================================================
  Subroutine updateInternalEnergy3T(eint,presFL,presFR,presGL,presGR,presHL,presHR,FL,FR,GL,GR,HL,HR,&
       dx,dy,dz,dt,lam3Rad,Qohm)
    ...
    real, dimension(HY_P_FLUX:), intent(IN)    :: presFL,presFR,presGL,presGR,presHL,presHR
    real, dimension(HY_EINT_FLUX:), intent(IN) :: FL,FR,GL,GR,HL,HR
    real, intent(IN) :: dx,dy,dz,dt,lam3Rad,Qohm

    real :: duint
    real :: lam3L,lam3R
    real :: gB1,gB1r,gB0,gB0r

    gB1  = fB1  + fB0  ; gB0  = 0.0
    gB1r = fB1r + fB0r ; gB0r = 0.0

    duint = 0.0

    duint = duint + dt/dx*(FL(HY_EELE_FLUX)-FR(HY_EELE_FLUX)  &
                            + (0.5*gB1 * (presFL(HY_PMAT_FLUX)+presFR(HY_PMAT_FLUX)) *(FL(HY_VOLU_FLUX)-FR(HY_VOLU_FLUX))) &
                            - (0.5*fB2 * (presFL(HY_PMAT_FLUX)-presFR(HY_PMAT_FLUX)) *(FL(HY_VOLU_FLUX)+FR(HY_VOLU_FLUX))) )
    lam3L = max(1.d-199,(presFL(HY_P_FLUX) - presFL(HY_PMAT_FLUX))) / max(presFL(HY_PRAD_FLUX),1.d-199)
    lam3R = max(1.d-199,(presFR(HY_P_FLUX) - presFR(HY_PMAT_FLUX))) / max(presFR(HY_PRAD_FLUX),1.d-199)
    duint = duint + dt/dx*(FL(HY_ERAD_FLUX)-FR(HY_ERAD_FLUX)  &
                           + (  ( gB1r      *lam3rad      *    (FL(HY_VOLU_FLUX)-FR(HY_VOLU_FLUX))) &
                              + ((gB1r+gB0r)*(lam3L-lam3R)*0.5*(FL(HY_VOLU_FLUX)+FR(HY_VOLU_FLUX))))&
                             *(presFL(HY_PRAD_FLUX)+presFR(HY_PRAD_FLUX))*0.5 &
                           -  lam3Rad*fB2r * (presFL(HY_PRAD_FLUX)-presFR(HY_PRAD_FLUX)) &
                              *0.5*(FL(HY_VOLU_FLUX)+FR(HY_VOLU_FLUX)) )

    if (NDIM > 1) then
       duint = duint + dt/dy*(GL(HY_EELE_FLUX)-GR(HY_EELE_FLUX)  &
                            + (0.5*gB1 * (presGL(HY_PMAT_FLUX)+presGR(HY_PMAT_FLUX)) *(GL(HY_VOLU_FLUX)-GR(HY_VOLU_FLUX))) &
                            - (0.5*fB2 * (presGL(HY_PMAT_FLUX)-presGR(HY_PMAT_FLUX)) *(GL(HY_VOLU_FLUX)+GR(HY_VOLU_FLUX))) )
       lam3L = max(1.d-199,(presGL(HY_P_FLUX) - presGL(HY_PMAT_FLUX))) / max(presGL(HY_PRAD_FLUX),1.d-199)
       lam3R = max(1.d-199,(presGR(HY_P_FLUX) - presGR(HY_PMAT_FLUX))) / max(presGR(HY_PRAD_FLUX),1.d-199)
       duint = duint + dt/dy*(GL(HY_ERAD_FLUX)-GR(HY_ERAD_FLUX)  &
                              + (  ( gB1r      *lam3rad      *    (GL(HY_VOLU_FLUX)-GR(HY_VOLU_FLUX))) &
                                 + ((gB1r+gB0r)*(lam3L-lam3R)*0.5*(GL(HY_VOLU_FLUX)+GR(HY_VOLU_FLUX))))&
                                *(presGL(HY_PRAD_FLUX)+presGR(HY_PRAD_FLUX))*0.5 &
                              - lam3Rad*fB2r * (presGL(HY_PRAD_FLUX)-presGR(HY_PRAD_FLUX)) &
                                *0.5*(GL(HY_VOLU_FLUX)+GR(HY_VOLU_FLUX)) )

       if (NDIM > 2) then
          duint = duint + dt/dz*(HL(HY_EELE_FLUX)-HR(HY_EELE_FLUX)  &
                            + (0.5*gB1 * (presHL(HY_PMAT_FLUX)+presHR(HY_PMAT_FLUX)) *(HL(HY_VOLU_FLUX)-HR(HY_VOLU_FLUX))) &
                            - (0.5*fB2 * (presHL(HY_PMAT_FLUX)-presHR(HY_PMAT_FLUX)) *(HL(HY_VOLU_FLUX)+HR(HY_VOLU_FLUX))) )
          lam3L = max(1.d-199,(presHL(HY_P_FLUX) - presHL(HY_PMAT_FLUX))) / max(presHL(HY_PRAD_FLUX),1.d-199)
          lam3R = max(1.d-199,(presHR(HY_P_FLUX) - presHR(HY_PMAT_FLUX))) / max(presHR(HY_PRAD_FLUX),1.d-199)
          duint = duint + dt/dz*(HL(HY_ERAD_FLUX)-HR(HY_ERAD_FLUX)  &
                                 + (  ( gB1r      *lam3rad      *    (HL(HY_VOLU_FLUX)-HR(HY_VOLU_FLUX))) &
                                    + ((gB1r+gB0r)*(lam3L-lam3R)*0.5*(HL(HY_VOLU_FLUX)+HR(HY_VOLU_FLUX))))&
                                   *(presHL(HY_PRAD_FLUX)+presHR(HY_PRAD_FLUX))*0.5 &
                                 - lam3Rad*fB2r * (presHL(HY_PRAD_FLUX)-presHR(HY_PRAD_FLUX)) &
                                   *0.5*(HL(HY_VOLU_FLUX)+HR(HY_VOLU_FLUX)) )
       endif
    endif
  ...
  End Subroutine updateInternalEnergy3T
