!!****if* source/physics/Hydro/HydroMain/unsplit_rad/hy_uhd_getFaceFlux
!!
!! NAME
!!
!!  hy_uhd_getFaceFlux
...
subroutine hy_uhd_getFaceFlux ( blockID,blkLimits,blkLimitsGC,datasize,del,&
  ...
  use Hydro_data,                    ONLY : hy_nref,hy_kref,          &
                                            ...
                                            hy_EOSforRiemann
  use Hydro_data,                    ONLY : fE => hy_fEddiCorrEarly

#ifdef FLASH_UHD_3T
  use hy_uhd_MultiTempData, ONLY: hy_mtFactorB0,hy_mtFactorB1,fB2=>hy_mtFactorB2
  use hy_uhd_MultiTempData, ONLY: hy_mtFactorBrad0,hy_mtFactorBrad1,fB2r=>hy_mtFactorBrad2
#endif
     ...
     do j=blkLimits(LOW,JAXIS)-tranDir(DIR_Y),blkLimits(HIGH,JAXIS)+tranDir(DIR_Y)+normDir(DIR_Y)
        do i=blkLimits(LOW,IAXIS)-tranDir(DIR_X),blkLimits(HIGH,IAXIS)+tranDir(DIR_X)+normDir(DIR_X)
           ...
           if (hy_updateHydroFluxes) then
              ...
              VL(HY_DENS:HY_END_VARS-kGrav)=scrchFaceXPtr(HY_P01_FACEXPTR_VAR:HY_P01_FACEXPTR_VAR+HY_VARINUMMAX-2,i-1,j,k)
              VR(HY_DENS:HY_END_VARS-kGrav)=scrchFaceXPtr(HY_N01_FACEXPTR_VAR:HY_N01_FACEXPTR_VAR+HY_VARINUMMAX-2,i,  j,k)
...
!!$ MULTITEMP AFTERRIEMANN STUFF
                 ftot =  VL(HY_PMAT) + VL(HY_PRAD)
#ifdef DEBUG_FEB2015
                 print*,'VL:',i,xflux(HY_DENS_FLUX,i,j,k),VL(HY_PMAT),VR(HY_PMAT),VL(HY_PRAD),VR(HY_PRAD)
#endif
                 xflux(HY_ENER_FLUX,i,j,k) = xflux(HY_ENER_FLUX,i,j,k) + (1.0-fE)*xflux(HY_VOLU_FLUX,i,j,k)*VL(HY_EDDI)
                 xflux(HY_EINT_FLUX,i,j,k) = xflux(HY_EINT_FLUX,i,j,k) +          xflux(HY_VOLU_FLUX,i,j,k)*VL(HY_EDDI)
                 xflux(HY_ERAD_FLUX,i,j,k) = xflux(HY_ERAD_FLUX,i,j,k) +          xflux(HY_VOLU_FLUX,i,j,k)*VL(HY_EDDI)
                 xflux(HY_PMAT_FLUX,i,j,k) = (VL(HY_PMAT) * xflux(HY_P_FLUX,i,j,k)) !/ ftot ! (VL(HY_GAME)-1.0) * (VL(HY_EELE) + VL(HY_EION))*VL(HY_DENS)
                 xflux(HY_PRAD_FLUX,i,j,k) = (VL(HY_PRAD) * xflux(HY_P_FLUX,i,j,k)) !/ ftot
                 lambdaEr         =  (1.0 - VL(HY_PMAT)) * xflux(HY_P_FLUX,i,j,k)
                 !! Thus lambda can be recovered from 'fluxes' as follows:
                 !! lambda = (xflux(HY_P_FLUX,i,j,k) - xflux(HY_PMAT_FLUX,i,j,k)) / xflux(HY_PRAD_FLUX,i,j,k)

                 xflux(HY_EELE_FLUX,i,j,k) = xflux(HY_EELE_FLUX,i,j,k) + fB2 *xflux(HY_PMAT_FLUX,i,j,k) * xflux(HY_VOLU_FLUX,i,j,k)
                 xflux(HY_EINT_FLUX,i,j,k) = xflux(HY_EINT_FLUX,i,j,k) +        lambdaEr * xflux(HY_VOLU_FLUX,i,j,k)
                 xflux(HY_ERAD_FLUX,i,j,k) = xflux(HY_ERAD_FLUX,i,j,k) + fB2r * lambdaEr * xflux(HY_VOLU_FLUX,i,j,k)
              else
                 xflux(HY_EELE_FLUX:HY_ERAD_FLUX,i,j,k) = xflux(HY_DENS_FLUX,i,j,k)*VR(HY_EELE:HY_ERAD)
                 ftot =  VR(HY_PMAT) + VR(HY_PRAD)
#ifdef DEBUG_FEB2015
                 print*,'VR:',i,xflux(HY_DENS_FLUX,i,j,k),VL(HY_PMAT),VR(HY_PMAT),VL(HY_PRAD),VR(HY_PRAD)
#endif
                 xflux(HY_ENER_FLUX,i,j,k) = xflux(HY_ENER_FLUX,i,j,k) + (1.0-fE)*xflux(HY_VOLU_FLUX,i,j,k)*VR(HY_EDDI)
                 xflux(HY_EINT_FLUX,i,j,k) = xflux(HY_EINT_FLUX,i,j,k) +          xflux(HY_VOLU_FLUX,i,j,k)*VR(HY_EDDI)
                 xflux(HY_ERAD_FLUX,i,j,k) = xflux(HY_ERAD_FLUX,i,j,k) +          xflux(HY_VOLU_FLUX,i,j,k)*VR(HY_EDDI)
                 xflux(HY_PMAT_FLUX,i,j,k) = VR(HY_PMAT) * xflux(HY_P_FLUX,i,j,k) !/ ftot ! (VR(HY_GAME)-1.0) * (VR(HY_EELE) + VR(HY_EION))*VR(HY_DENS) 
                 xflux(HY_PRAD_FLUX,i,j,k) = VR(HY_PRAD) * xflux(HY_P_FLUX,i,j,k) !/ ftot 
                 lambdaEr         =  (1.0 - VR(HY_PMAT)) * xflux(HY_P_FLUX,i,j,k)
                 xflux(HY_EELE_FLUX,i,j,k) = xflux(HY_EELE_FLUX,i,j,k) + fB2 *xflux(HY_PMAT_FLUX,i,j,k) * xflux(HY_VOLU_FLUX,i,j,k)
                 xflux(HY_EINT_FLUX,i,j,k) = xflux(HY_EINT_FLUX,i,j,k) +        lambdaEr * xflux(HY_VOLU_FLUX,i,j,k)
                 xflux(HY_ERAD_FLUX,i,j,k) = xflux(HY_ERAD_FLUX,i,j,k) + fB2r * lambdaEr * xflux(HY_VOLU_FLUX,i,j,k)
              endif
             ! if (abs(ftot-1.0)>0.001) print*,'Bad ftot=',ftot,' at i=',i
!!$ END OF MULTITEMP AFTERRIEMANN STUFF
  ...
  do k=blkLimits(LOW,KAXIS)-tranDir(DIR_Z),blkLimits(HIGH,KAXIS)+tranDir(DIR_Z)+normDir(DIR_Z)
     do j=blkLimits(LOW,JAXIS)-tranDir(DIR_Y),blkLimits(HIGH,JAXIS)+tranDir(DIR_Y)+normDir(DIR_Y)
        ...
        if (hy_updateHydroFluxes) then
           ...
           VL(HY_DENS:HY_END_VARS-kGrav)=scrchFaceYPtr(HY_P01_FACEYPTR_VAR:HY_P01_FACEYPTR_VAR+HY_VARINUMMAX-2,i,j-1,k)
           VR(HY_DENS:HY_END_VARS-kGrav)=scrchFaceYPtr(HY_N01_FACEYPTR_VAR:HY_N01_FACEYPTR_VAR+HY_VARINUMMAX-2,i,j,  k)
...
!!$ MULTITEMP AFTERRIEMANN STUFF
              ftot =  VL(HY_PMAT) + VL(HY_PRAD)
              yflux(HY_ENER_FLUX,i,j,k) = yflux(HY_ENER_FLUX,i,j,k) + (1.0-fE)*yflux(HY_VOLU_FLUX,i,j,k)*VL(HY_EDDI)
              yflux(HY_EINT_FLUX,i,j,k) = yflux(HY_EINT_FLUX,i,j,k) +          yflux(HY_VOLU_FLUX,i,j,k)*VL(HY_EDDI)
              yflux(HY_ERAD_FLUX,i,j,k) = yflux(HY_ERAD_FLUX,i,j,k) +          yflux(HY_VOLU_FLUX,i,j,k)*VL(HY_EDDI)
              yflux(HY_PMAT_FLUX,i,j,k) = VL(HY_PMAT) * yflux(HY_P_FLUX,i,j,k) !/ ftot ! (VL(HY_GAME)-1.0) * (VL(HY_EELE) + VL(HY_EION))*VL(HY_DENS)
              yflux(HY_PRAD_FLUX,i,j,k) = VL(HY_PRAD) * yflux(HY_P_FLUX,i,j,k) !/ ftot
              lambdaEr         =  (1.0 - VL(HY_PMAT)) * yflux(HY_P_FLUX,i,j,k)
              yflux(HY_EINT_FLUX,i,j,k) = yflux(HY_EINT_FLUX,i,j,k) + lambdaEr * yflux(HY_VOLU_FLUX,i,j,k)
              yflux(HY_ERAD_FLUX,i,j,k) = yflux(HY_ERAD_FLUX,i,j,k) + fB2r * lambdaEr * yflux(HY_VOLU_FLUX,i,j,k)
           else
              ftot =  VR(HY_PMAT) + VR(HY_PRAD)
              yflux(HY_EELE_FLUX:HY_ERAD_FLUX,i,j,k) = yflux(HY_DENS_FLUX,i,j,k)*VR(HY_EELE:HY_ERAD)
              yflux(HY_ENER_FLUX,i,j,k) = yflux(HY_ENER_FLUX,i,j,k) + (1.0-fE)*yflux(HY_VOLU_FLUX,i,j,k)*VR(HY_EDDI)
              yflux(HY_EINT_FLUX,i,j,k) = yflux(HY_EINT_FLUX,i,j,k) +          yflux(HY_VOLU_FLUX,i,j,k)*VR(HY_EDDI)
              yflux(HY_ERAD_FLUX,i,j,k) = yflux(HY_ERAD_FLUX,i,j,k) +          yflux(HY_VOLU_FLUX,i,j,k)*VR(HY_EDDI)
              yflux(HY_PMAT_FLUX,i,j,k) = VR(HY_PMAT) * yflux(HY_P_FLUX,i,j,k) !/ ftot ! (VR(HY_GAME)-1.0) * (VR(HY_EELE) + VR(HY_EION))*VR(HY_DENS)  
              yflux(HY_PRAD_FLUX,i,j,k) = VR(HY_PRAD) * yflux(HY_P_FLUX,i,j,k) !/ ftot 
              lambdaEr         =  (1.0 - VR(HY_PMAT)) * yflux(HY_P_FLUX,i,j,k)
              yflux(HY_EINT_FLUX,i,j,k) = yflux(HY_EINT_FLUX,i,j,k) + lambdaEr * yflux(HY_VOLU_FLUX,i,j,k)
              yflux(HY_ERAD_FLUX,i,j,k) = yflux(HY_ERAD_FLUX,i,j,k) + fB2r * lambdaEr * yflux(HY_VOLU_FLUX,i,j,k)
           endif
           yflux(HY_EELE_FLUX,i,j,k) = yflux(HY_EELE_FLUX,i,j,k) + fB2 * yflux(HY_PMAT_FLUX,i,j,k) * yflux(HY_VOLU_FLUX,i,j,k)
!!$ END OF MULTITEMP AFTERRIEMANN STUFF
#endif
  ...
  do k=blkLimits(LOW,KAXIS)-tranDir(DIR_Z),blkLimits(HIGH,KAXIS)+tranDir(DIR_Z)+normDir(DIR_Z)
     do j=blkLimits(LOW,JAXIS)-tranDir(DIR_Y),blkLimits(HIGH,JAXIS)+tranDir(DIR_Y)+normDir(DIR_Y)
        ...
        if (hy_updateHydroFluxes) then
           ...
           VL(HY_DENS:HY_END_VARS-kGrav)=scrchFaceZPtr(HY_P01_FACEZPTR_VAR:HY_P01_FACEZPTR_VAR+HY_VARINUMMAX-2,i,j,k-1)
           VR(HY_DENS:HY_END_VARS-kGrav)=scrchFaceZPtr(HY_N01_FACEZPTR_VAR:HY_N01_FACEZPTR_VAR+HY_VARINUMMAX-2,i,j,k)
...
!!$ MULTITEMP AFTERRIEMANN STUFF
              ftot =  VL(HY_PMAT) + VL(HY_PRAD)
              zflux(HY_ENER_FLUX,i,j,k) = zflux(HY_ENER_FLUX,i,j,k) + (1.0-fE)*zflux(HY_VOLU_FLUX,i,j,k)*VL(HY_EDDI)
              zflux(HY_EINT_FLUX,i,j,k) = zflux(HY_EINT_FLUX,i,j,k) +          zflux(HY_VOLU_FLUX,i,j,k)*VL(HY_EDDI)
              zflux(HY_ERAD_FLUX,i,j,k) = zflux(HY_ERAD_FLUX,i,j,k) +          zflux(HY_VOLU_FLUX,i,j,k)*VL(HY_EDDI)
              zflux(HY_PMAT_FLUX,i,j,k) = VL(HY_PMAT) * zflux(HY_P_FLUX,i,j,k) !/ ftot! (VL(HY_GAME)-1.0) * (VL(HY_EELE) + VL(HY_EION))*VL(HY_DENS)
              zflux(HY_PRAD_FLUX,i,j,k) = VL(HY_PRAD) * zflux(HY_P_FLUX,i,j,k) !/ ftot
              lambdaEr         =  (1.0 - VL(HY_PMAT)) * zflux(HY_P_FLUX,i,j,k)
              zflux(HY_EINT_FLUX,i,j,k) = zflux(HY_EINT_FLUX,i,j,k) + lambdaEr * zflux(HY_VOLU_FLUX,i,j,k)
              zflux(HY_ERAD_FLUX,i,j,k) = zflux(HY_ERAD_FLUX,i,j,k) + fB2r * lambdaEr * zflux(HY_VOLU_FLUX,i,j,k)
           else
              ftot =  VR(HY_PMAT) + VR(HY_PRAD)
              zflux(HY_EELE_FLUX:HY_ERAD_FLUX,i,j,k) = zflux(HY_DENS_FLUX,i,j,k)*VR(HY_EELE:HY_ERAD)
              zflux(HY_ENER_FLUX,i,j,k) = zflux(HY_ENER_FLUX,i,j,k) + (1.0-fE)*zflux(HY_VOLU_FLUX,i,j,k)*VR(HY_EDDI)
              zflux(HY_EINT_FLUX,i,j,k) = zflux(HY_EINT_FLUX,i,j,k) +          zflux(HY_VOLU_FLUX,i,j,k)*VR(HY_EDDI)
              zflux(HY_ERAD_FLUX,i,j,k) = zflux(HY_ERAD_FLUX,i,j,k) +          zflux(HY_VOLU_FLUX,i,j,k)*VR(HY_EDDI)
              zflux(HY_PMAT_FLUX,i,j,k) = VR(HY_PMAT) * zflux(HY_P_FLUX,i,j,k) !/ ftot ! (VR(HY_GAME)-1.0) * (VR(HY_EELE) + VR(HY_EION))*VR(HY_DENS)
              zflux(HY_PRAD_FLUX,i,j,k) = VR(HY_PRAD) * zflux(HY_P_FLUX,i,j,k) !/ ftot
              lambdaEr         =  (1.0 - VR(HY_PMAT)) * zflux(HY_P_FLUX,i,j,k)
              zflux(HY_EINT_FLUX,i,j,k) = zflux(HY_EINT_FLUX,i,j,k) + lambdaEr * zflux(HY_VOLU_FLUX,i,j,k)
              zflux(HY_ERAD_FLUX,i,j,k) = zflux(HY_ERAD_FLUX,i,j,k) + fB2r * lambdaEr * zflux(HY_VOLU_FLUX,i,j,k)
           endif
           zflux(HY_EELE_FLUX,i,j,k) = zflux(HY_EELE_FLUX,i,j,k) + fB2 * zflux(HY_PMAT_FLUX,i,j,k) * zflux(HY_VOLU_FLUX,i,j,k)
!!$ END OF MULTITEMP AFTERRIEMANN STUFF
#endif
