!!****if* source/physics/Hydro/HydroMain/unsplit_rad/hy_uhd_prim2flx
!!
!! NAME
!!
!!  hy_uhd_prim2flx
...
!!                   real(IN),dimension(*) :: V(HY_EDDI),
!!                   real(OUT)             :: F(HY_VARINUM1))
!!
!! ARGUMENTS
!!
!! dir- directional index
!! V  - primitive variables  + GAMC,GAME,...,EDDI
!! F  - flux
!!
!! DESCRIPTION
!!
...
#include "UHD.h"
  ...
  real  :: u2,E
  real  :: B2,UB,ptot
  real  :: eRadCorr ! to take account of difference between (3-f)/2 and lambda+1
...
#ifdef FLASH_UHD_3T
#if HY_EDDI <= HY_VARINUM4
  eRadCorr = fE * V(HY_EDDI)
#endif
#endif
  ...
  if (dir==DIR_X) then
     ...
     F(F05ENER_FLUX) =  (E+V(HY_PRES)+eRadCorr)*V(HY_VELX)
#else
     ...
     F(F05ENER_FLUX) = (E+ptot+eRadCorr)*V(HY_VELX)-V(HY_MAGX)*UB
#ifdef FLASH_USM_MHD
     F(F06MAGX_FLUX) = 0.0
#endif
  ...
  elseif (dir==DIR_Y) then
     ...
     F(F04ZMOM_FLUX) = F(F01DENS_FLUX)*V(HY_VELZ)
     F(F05ENER_FLUX) =  (E+V(HY_PRES)+eRadCorr)*V(HY_VELY)
#else
     ...
     F(F05ENER_FLUX) = (E+ptot+eRadCorr)*V(HY_VELY)-V(HY_MAGY)*UB
  ...
  elseif (dir==DIR_Z) then
     ...
     F(F05ENER_FLUX) =  (E+V(HY_PRES)+eRadCorr)*V(HY_VELZ)
#else
     ...
     F(F05ENER_FLUX) = (E+ptot+eRadCorr)*V(HY_VELZ)-V(HY_MAGZ)*UB
