!!****if* source/physics/Hydro/HydroMain/unsplit_rad/hy_uhd_DataReconstructNormalDir_PPM
!!
!! NAME
!!
...
#include "UHD.h"
  ...
  IF (.not. TransUpdateOnly) THEN
#if NGUARD > 4
     Vmmm(HY_PRAD)         = Data1D(ERAD_VAR,    iBeg) * Data1D(DENS_VAR,    iBeg) / 3.0
     Vmmm(HY_FLXL)         = Data1D(FLLM_VAR,           iBeg)
     Vmmm(HY_EDDI)         =(Data1D(EDDI_VAR,    iBeg) - Data1D(FLLM_VAR,    iBeg)) * Vmmm(HY_PRAD)
     Vmmm(HY_PMAT)         = Data1D(PION_VAR,    iBeg) + Data1D(PELE_VAR,    iBeg)
     Vmmm(HY_PMAT)         = Vmmm(HY_PMAT) / Vmm(HY_PRES)
     Vmmm(HY_PRAD)         = Vmmm(HY_PRAD) / Vmm(HY_PRES)

     Vppp(HY_PRAD)         = Data1D(ERAD_VAR,  6+iBeg) * Data1D(DENS_VAR,  6+iBeg) / 3.0
     Vppp(HY_FLXL)         = Data1D(FLLM_VAR,         6+iBeg)
     Vppp(HY_EDDI)         =(Data1D(EDDI_VAR,  6+iBeg) - Data1D(FLLM_VAR,  6+iBeg)) * Vppp(HY_PRAD)
     Vppp(HY_PMAT)         = Data1D(PION_VAR,  6+iBeg) + Data1D(PELE_VAR,  6+iBeg)
     Vppp(HY_PMAT)         = Vppp(HY_PMAT) / Vpp(HY_PRES)
     Vppp(HY_PRAD)         = Vppp(HY_PRAD) / Vpp(HY_PRES)
#endif
...
#ifdef DEBUG_FEB2015
  print*,'PPM HY_PRES',Vmm(HY_PRES),Vm(HY_PRES),Vc(HY_PRES),Vp(HY_PRES),Vpp(HY_PRES)
  print*,'PPM HY_PMAT',Vmm(HY_PMAT),Vm(HY_PMAT),Vc(HY_PMAT),Vp(HY_PMAT),Vpp(HY_PMAT)
#endif
#endif
...
!!$ delbar0=0.; delbarN=0.; delbarP=0.
  ...
  if (Vc(HY_PRES) .LE. 0.0) then
     print*,'Vc(HY_PRES) is',Vc(HY_PRES),' in PPM #1!!!!!'
     print*,'Vc(:) =',Vc
  end if
  call hy_uhd_eigenParameters&
       (Vc(HY_DENS:HY_VARINUM2),dir,uN,cf&
  ...
  if (Vp(HY_PRES) .LE. 0.0) then
     print*,'Vp(HY_PRES) is',Vp(HY_PRES),' in PPM #2!!!!!'
     print*,'Vp(:) =',Vp
  end if
  call hy_uhd_eigenParameters&
       (Vp(HY_DENS:HY_VARINUM2),dir,uN,cf&
#if defined(FLASH_USM_MHD) || defined(FLASH_UGLM_MHD)
       ,C_alfn=ca,C_slow=cs,A_f=af,A_s=as,B_beta=beta&
#ifdef FLASH_UGLM_MHD
       ,C_hyp=hyp&
  ...
  if (Vm(HY_PRES) .LE. 0.0) then
     print*,'Vm(HY_PRES) is',Vm(HY_PRES),' in PPM #3!!!!!'
     print*,'Vm(:) =',Vm
  end if
  call hy_uhd_eigenParameters&
       (Vm(HY_DENS:HY_VARINUM2),dir,uN,cf&
#if defined(FLASH_USM_MHD) || defined(FLASH_UGLM_MHD)
       ,C_alfn=ca,C_slow=cs,A_f=af,A_s=as,B_beta=beta&
#ifdef FLASH_UGLM_MHD
       ,C_hyp=hyp&
  ...
  call hy_uhd_eigenVector&
     ...
     if (.not. hy_upwindTVD) then        
        ...
        !! lambdaNN
        if (Vmm(HY_PRES) .LE. 0.0) then
           print*,'Vmm(HY_PRES) is',Vmm(HY_PRES),' in PPM #4!!!!!'
           print*,'Vmm(:) =',Vmm
        end if
        call hy_uhd_eigenParameters&
             (Vmm(HY_DENS:HY_VARINUM2),dir,uN,cf&
#if defined(FLASH_USM_MHD) || defined(FLASH_UGLM_MHD)
             ,C_alfn=ca,C_slow=cs,A_f=af,A_s=as,B_beta=beta&
#ifdef FLASH_UGLM_MHD
             ,C_hyp=hyp&
        ...
        !! lambdaPP
        if (Vpp(HY_PRES) .LE. 0.0) then
           print*,'Vpp(HY_PRES) is',Vpp(HY_PRES),' in PPM #5!!!!!'
           print*,'Vpp(:) =',Vpp
        end if
        call hy_uhd_eigenParameters&
             (Vpp(HY_DENS:HY_VARINUM2),dir,uN,cf&
#if defined(FLASH_USM_MHD) || defined(FLASH_UGLM_MHD)
             ,C_alfn=ca,C_slow=cs,A_f=af,A_s=as,B_beta=beta&
#ifdef FLASH_UGLM_MHD
             ,C_hyp=hyp&
