!!****if* source/physics/Hydro/HydroMain/unsplit_rad/threadWithinBlock/hy_uhd_unsplitUpdate
!!
!! NAME
!!
...
#include "UHD.h"
  ...
  Subroutine hy_uhd_unsplitUpdate(blockID,rangeSwitch,dt,del,dataSize,blkLimits,&
    ...
    use Hydro_data,           ONLY : hy_smalldens,hy_order,hy_irenorm,hy_numXN, &
                                     ...
                                     hy_conserveAngMom, hy_smallE,hy_threadWithinBlock, &
                                     hy_irenorm
    use Hydro_data,           ONLY : fP => hy_fPresInMomFlux
    use Hydro_data,           ONLY : hy_MOM_PHI,hy_MOM_PHI_FLUX,&
                                     hy_MAG_PHI, hy_MAG_PHI_FLUX, hy_MAG_ZI, hy_MOM_THT
#ifdef FLASH_USM_MHD
...
!    real, dimension(dataSize(IAXIS),dataSize(JAXIS),dataSize(KAXIS)) :: faceAreas, cellVolumes
    real, dimension(blkLimits(LOW,IAXIS):blkLimits(HIGH,IAXIS)+2,&
  ...
  !parallel region for both hydro and mhd applications. Not used.
  logical :: hy_mref, hy_hallVelocity, &
             hy_useMagneticResistivity, hy_conserveAngField
#endif
    ...
    !$omp shared(xflux,yflux,zflux,scrch_Ptr,U,hy_conserveangmom,hy_hallVelocity,&
    !$omp fP,&
    !$omp dx,dy,dz,dt,gravX,gravY,gravZ,SpOld,UOld,hy_useMagneticResistivity,&
    !$omp hy_smalldens,hy_smallE,hy_forceHydroLimit,hy_order,hy_conserveAngField,&
    !$omp blockID,del,dataSize,blkLimits,hy_mref,Jp,Jm,&
    !$omp hy_geometry,hdt,faceAreas,cellVolumes,rangeSwitch,hy_useauxeinteqn,&
    !$omp hy_fullSpecMsFluxHandling, &
    !$omp hy_MOM_PHI,hy_MOM_PHI_FLUX,&
    !$omp hy_MAG_PHI, hy_MAG_PHI_FLUX, hy_MAG_ZI, hy_MOM_THT, &
    !$omp hy_eswitch) &
    !$omp private(FL,FR,GL,GR,HL,HR,IntEner,newEint,tempPres,i,j,k,U0,&
    !$omp xCenter,xLeft,xRight,yCenter,Sgeo,Sphys,zmagStar,&
    !$omp densStar,presStar,pmomStar,tmomStar,xmomStar,xmagStar,pmagStar,&
    !$omp ky,kz,&
    !$omp alpha,dx_sph,leftFac,rghtFac,sumspecies,ispu,isph,&
    !$omp newDens,specialForInterior,&
    !$omp ekin,eint,abar,zbar,Qohm,speciesArr,res_eta,eta_loc,inv_dvrm,&
    !$omp inv_dvrp,jym,jyp,res_source,dxbzm,dxbzp)
...
#  endif
                      ...
                      if (hy_conserveAngMom) then
                         FR(hy_MOM_PHI_FLUX) = FR(hy_MOM_PHI_FLUX)*rghtFac
                         FL(hy_MOM_PHI_FLUX) = FL(hy_MOM_PHI_FLUX)*leftFac
                      endif
                      ...
                      if (.not. hy_conserveAngField) then
                         FR(hy_MAG_PHI_FLUX) = FR(hy_MAG_PHI_FLUX)*rghtFac
                         FL(hy_MAG_PHI_FLUX) = FL(hy_MAG_PHI_FLUX)*leftFac
                      endif
#endif
                   endif
                ...
                if (hy_geometry /= CARTESIAN) then
                   ...
                   densStar = U0(HY_DENS)
                   xmomStar = U0(HY_XMOM)
                   pmomStar = U0(hy_MOM_PHI)

#if defined(FLASH_USM_MHD) || defined(FLASH_UGLM_MHD)
                   xmagStar = U0(HY_MAGX)
                   pmagStar = U0(hy_MAG_PHI)
                   zmagStar = U0(hy_MAG_ZI)
#endif 

                   Sgeo(HY_XMOM) = (pmomStar**2/densStar + fP*alpha*presStar) / xCenter(i)!T phi,phi
                   Sgeo(hy_MOM_PHI) = (pmomStar*xmomStar/densStar) / xCenter(i)!T phi,r
                 ...
                 ! P* is the total Pressure
                   ...
                   Sgeo(hy_MOM_PHI) = Sgeo(hy_MOM_PHI) - pmagStar*xmagStar / xCenter(i)

                   if (hy_geometry == CYLINDRICAL .and. hy_conserveAngMom) then
                      Sgeo(hy_MOM_PHI) = 0.0
                   endif

                   Sgeo(hy_MAG_PHI) = - ((pmomStar/densStar) * xmagStar - &
                   ...
                   if (hy_geometry == CYLINDRICAL .and. &
                     ...
                     !! do not use it with magnetic resistivity.
                      ...
                      Sgeo(hy_MAG_PHI) = 0.0!- ((pmomStar/densStar) * xmagStar - &
                   ...
                   if (hy_geometry == CYLINDRICAL .and. &
                      ...
                      if (.not. hy_conserveAngField) then
                         Sgeo(hy_MAG_PHI) = Sgeo(hy_MAG_PHI) + res_source(i,j,k) 
                      endif
                   endif
#endif
                   Sgeo(hy_MOM_PHI) = - Sgeo(hy_MOM_PHI)


                   if (hy_geometry == SPHERICAL) then
                      tmomStar = U0(hy_MOM_THT) 
                ...
                call updateConservedVariable&
                     (U0(HY_DENS:HY_DENS+HY_VARINUM-1),&
                      ...
                      gravX(i,j,k),gravY(i,j,k),gravZ(i,j,k),dx,dy,dz,dt,Sgeo,Sphys)
                ...
                if (hy_fullSpecMsFluxHandling .OR. specialForInterior) then
                   sumSpecies = 0.
                   do ispu = SPECIES_BEGIN, MASS_SCALARS_END !SPECIES_END
  ...
  CONTAINS
    ...
    !$omp dx,dy,dz,dt,gravX,gravY,gravZ,SpOld,UOld,&
    !$omp hy_order,&
    !$omp blockID,del,dataSize,blkLimits,&
    !$omp hy_geometry,hdt,faceAreas,cellVolumes,&
    !$omp hy_fullSpecMsFluxHandling) &
    !$omp private(FL,FR,GL,GR,HL,HR,i,j,k,&
    !$omp xCenter,xLeft,xRight,yCenter,&
    !$omp ky,kz,&
    !$omp leftFac,rghtFac,ispu,isph)
          ...
          do i=imin,imax
             ...
             if (U(BDRY_VAR,i,j,k) .LE. 0.0) then
                ...
                if (hy_geometry /= CARTESIAN) then
                   ...
                   if (hy_geometry == CYLINDRICAL .OR. hy_geometry == SPHERICAL) then
                      if (hy_fullSpecMsFluxHandling) then
                         do ispu = SPECIES_BEGIN, MASS_SCALARS_END
                            isph= ispu-NPROP_VARS
                            FR(HY_END_FLUX+isph) = FR(HY_END_FLUX+isph)*rghtFac
