!!****if* source/physics/Hydro/HydroMain/unsplit_rad/hy_uhd_getRiemannState
!!
!! NAME
!!
     ...
     do k=k0-2-(k3*kUSM-kHydro)*k3,kmax+2+(k3*kUSM-kHydro)*k3
        ...
        do j=j0-2-(k3*kUSM-kHydro)*k2,jmax+2+(k3*kUSM-kHydro)*k2
           ...
           !(i.e., cross derivatives in x,y, & z)
...
#ifdef EDDI_VAR
                       ,(U(EDDI_VAR,i,j,k)-U(FLLM_VAR,i,j,k))*(U(ERAD_VAR,i,j,k)*U(DENS_VAR,i,j,k)/3.0) &
#else
                       ,U(FLLM_VAR,i,j,k) &
#endif
                       ,(U(ERAD_VAR,i,j,k)*U(DENS_VAR,i,j,k)/3.0) / U(PRES_VAR,i,j,k) &
                       ,(U(PION_VAR,i,j,k)+U(PELE_VAR,i,j,k)) / U(PRES_VAR,i,j,k) &
#endif
                       ,U(EELE_VAR,i,j,k) &
                       ,U(EION_VAR,i,j,k) &
                       ,U(ERAD_VAR,i,j,k) &
              ...
              if (ANY(U(PRES_VAR,i-2:i+2,j,k) .LE. 0.0)) then
                 print*,'U(PRES_VAR,i) is',U(PRES_VAR,i-2:i+2,j,k),' around i=',i,',blk',blockID,' !!!!!'
              end if
              order = hy_order
...
#define DIRS DIR_X:dirLast
                 ...
                 Wn(HY_DENS,  DIRS ) = Wn(HY_DENS,  DIRS ) + hdt * sGeo_dens
                 Wn(HY_VELX,  DIR_X) = Wn(HY_VELX,  DIR_X) + hdt * sGeo_velx
                 Wn(HY_velPhi,DIR_X) = Wn(HY_velPhi,DIR_X) + hdt * sGeo_velp
                 Wn(HY_PRES,  DIRS ) = Wn(HY_PRES,  DIRS ) + hdt * sGeo_pres
                 Wn(HY_EINT,  DIRS ) = Wn(HY_EINT,  DIRS ) + hdt * sGeo_eint
#if NDIM > 1
                 if (hy_geometry == SPHERICAL) &
                      Wn(HY_velTht,DIRS ) = Wn(HY_velTht,DIRS ) + hdt * sGeo_velt
#endif
#if defined(FLASH_USM_MHD) || defined(FLASH_UGLM_MHD)
                 ...
                 Wp(HY_DENS,  DIRS ) = Wp(HY_DENS,  DIRS ) + hdt * sGeo_dens
                 Wp(HY_VELX,  DIR_X) = Wp(HY_VELX,  DIR_X) + hdt * sGeo_velx
                 Wp(HY_velPhi,DIR_X) = Wp(HY_velPhi,DIR_X) + hdt * sGeo_velp
                 Wp(HY_PRES,  DIRS ) = Wp(HY_PRES,  DIRS ) + hdt * sGeo_pres
                 Wp(HY_EINT,  DIRS ) = Wp(HY_EINT,  DIRS ) + hdt * sGeo_eint
#if NDIM > 1
                 if (hy_geometry == SPHERICAL) &
                      Wp(HY_velTht,DIRS ) = Wp(HY_velTht,DIRS ) + hdt * sGeo_velt
#endif
#if defined(FLASH_USM_MHD) || defined(FLASH_UGLM_MHD)
             ...
             if(.NOT.TransX_updateOnly) scrchFaceXPtr(HY_P01_FACEXPTR_VAR:HY_P01_FACEXPTR_VAR+HY_VARINUMMAX-2,i,j,k)&
                   = Wp(HY_DENS:HY_END_VARS-kGrav,DIR_X)

              if(.NOT.TransX_updateOnly) scrchFaceXPtr(HY_N01_FACEXPTR_VAR:HY_N01_FACEXPTR_VAR+HY_VARINUMMAX-2,i,j,k)&
...
#endif
              !! Store Riemann states to scratch arrays
              if(.NOT.TransY_updateOnly) scrchFaceYPtr(HY_P01_FACEYPTR_VAR:HY_P01_FACEYPTR_VAR+HY_VARINUMMAX-2,i,j,k)&
                   = Wp(HY_DENS:HY_END_VARS-kGrav,DIR_Y)

              if(.NOT.TransY_updateOnly) scrchFaceYPtr(HY_N01_FACEYPTR_VAR:HY_N01_FACEYPTR_VAR+HY_VARINUMMAX-2,i,j,k)&
...
#endif
              !! Store Riemann states to scratch arrays
              if(.NOT.TransZ_updateOnly) scrchFaceZPtr(HY_P01_FACEZPTR_VAR:HY_P01_FACEZPTR_VAR+HY_VARINUMMAX-2,i,j,k)&
                   = Wp(HY_DENS:HY_END_VARS-kGrav,DIR_Z)

              if(.NOT.TransZ_updateOnly) scrchFaceZPtr(HY_N01_FACEZPTR_VAR:HY_N01_FACEZPTR_VAR+HY_VARINUMMAX-2,i,j,k)&
...
#endif
                 scrchFaceXPtr(HY_P01_FACEXPTR_VAR:HY_P01_FACEXPTR_VAR+HY_VARINUMMAX-2,i,j,k)=&
                      scrchFaceXPtr(HY_P01_FACEXPTR_VAR:HY_P01_FACEXPTR_VAR+HY_VARINUMMAX-2,i,j,k)&
                      +(TransFluxYZ(HY_DENS:HY_END_VARS-kGrav)+TransFluxZY(HY_DENS:HY_END_VARS-kGrav))*dt2dydz6

                 scrchFaceXPtr(HY_N01_FACEXPTR_VAR:HY_N01_FACEXPTR_VAR+HY_VARINUMMAX-2,i,j,k)=&
                      scrchFaceXPtr(HY_N01_FACEXPTR_VAR:HY_N01_FACEXPTR_VAR+HY_VARINUMMAX-2,i,j,k)&
                      +(TransFluxYZ(HY_DENS:HY_END_VARS-kGrav)+TransFluxZY(HY_DENS:HY_END_VARS-kGrav))*dt2dydz6
...
#endif
                 scrchFaceYPtr(HY_P01_FACEYPTR_VAR:HY_P01_FACEYPTR_VAR+HY_VARINUMMAX-2,i,j,k)=&
                      scrchFaceYPtr(HY_P01_FACEYPTR_VAR:HY_P01_FACEYPTR_VAR+HY_VARINUMMAX-2,i,j,k)&
                      + (TransFluxZX(HY_DENS:HY_END_VARS-kGrav)+TransFluxXZ(HY_DENS:HY_END_VARS-kGrav))*dt2dzdx6

                 scrchFaceYPtr(HY_N01_FACEYPTR_VAR:HY_N01_FACEYPTR_VAR+HY_VARINUMMAX-2,i,j,k)=&
                      scrchFaceYPtr(HY_N01_FACEYPTR_VAR:HY_N01_FACEYPTR_VAR+HY_VARINUMMAX-2,i,j,k)&
                      + (TransFluxZX(HY_DENS:HY_END_VARS-kGrav)+TransFluxXZ(HY_DENS:HY_END_VARS-kGrav))*dt2dzdx6
...
!!$                         Vc(HY_DENS:HY_END_VARS-kGrav)
                 ...
                 scrchFaceZPtr(HY_P01_FACEZPTR_VAR:HY_P01_FACEZPTR_VAR+HY_VARINUMMAX-2,i,j,k)=&
                      scrchFaceZPtr(HY_P01_FACEZPTR_VAR:HY_P01_FACEZPTR_VAR+HY_VARINUMMAX-2,i,j,k)&
                      + (TransFluxXY(HY_DENS:HY_END_VARS-kGrav)+TransFluxYX(HY_DENS:HY_END_VARS-kGrav))*dt2dxdy6

                 scrchFaceZPtr(HY_N01_FACEZPTR_VAR:HY_N01_FACEZPTR_VAR+HY_VARINUMMAX-2,i,j,k)=&
                      scrchFaceZPtr(HY_N01_FACEZPTR_VAR:HY_N01_FACEZPTR_VAR+HY_VARINUMMAX-2,i,j,k)&
                      + (TransFluxXY(HY_DENS:HY_END_VARS-kGrav)+TransFluxYX(HY_DENS:HY_END_VARS-kGrav))*dt2dxdy6
