!!****if* source/physics/Hydro/HydroMain/unsplit/MHD_StaggeredMesh/threadBlockList/hy_uhd_unsplit
!!
!! NAME
!!
!!  hy_uhd_unsplit
...
#include "UHD.h"
  ...
  !$omp parallel if (hy_threadBlockList .AND. blockCount > 1) &
  !$omp default(none) &
  !$omp private(updateMode,i,blockID,del,blkLimits,blkLimitsGC,datasize,&
  !$omp neighLev,myRefine,&
  !$omp flx,fly,flz,gravX,gravY,gravZ,U,&
  !$omp scrchFaceXPtr,scrchFaceYPtr,scrchFaceZPtr,scrch_Ptr,&
  !$omp hy_spcr,hy_spcl,hy_spcsig,&
  !$omp faceAreas,halftimeadvance) &
  !$omp shared(blockCount,blockList,gcMask,gcMaskLogged,blockNeedsFluxCorrect,blockMustStoreFluxes,&
  !$omp dt,dtOld,hy_units,&
  !$omp hy_unsplitEosMode,hy_useGravity,hy_gref,hy_fluxCorrect,hy_order,&
  !$omp hy_updateHydroFluxes,hy_eosModeAfter,&
  !$omp hy_geometry,hy_forcehydrolimit,&
  !$omp hy_killdivb,hy_gcmasksize,&
  !$omp hy_usebiermann,hy_usebiermann1t,hy_biermannsource,hy_fluxCorVars,hy_fullRiemannStateArrays,&
  !$omp hy_fullSpecMsFluxHandling,hy_threadWithinBlock)
...
#endif
     if (updateMode == UPDATE_ALL) then ! done with updating all cells of this block?
#if NFACE_VARS > 0
        !! ************************************************************************
...
!$omp end critical        
  ...
  if (hy_fluxCorrect) then
     ...
     do i = 1,blockCount        !LOOP 4
        ...
        !$omp scrchFaceXPtr,scrchFaceYPtr,scrchFaceZPtr,faceAreas,&
        !$omp blockID,blkLimits,dataSize,dt,dtOld,hy_fullRiemannStateArrays,&
        !$omp hy_geometry,hy_fluxCorVars)
        ...
        if (hy_geometry /= CARTESIAN) then
           !$omp sections private(faceAreas)
           !$omp section
           call Grid_getBlkData(blockID,CELL_FACEAREA,ILO_FACE, EXTERIOR, &
                                (/1,1,1/), faceAreas, datasize)
