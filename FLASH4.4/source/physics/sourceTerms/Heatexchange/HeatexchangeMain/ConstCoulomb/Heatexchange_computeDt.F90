!!****if* source/physics/sourceTerms/Heatexchange/HeatexchangeMain/ConstCoulomb/Heatexchange_computeDt
!!
!! NAME
!!
...
#include "Flash.h"
  ...
  real                       :: dynamicZ, relA, Ye, ni, nePerDens, ionMassInUAmu, memi
...
!!$              Zinv = 1.0/dynamicZ; ZpInv = 1.0/Zp
           ...
           Ye = dynamicZ / relA
           ni = solnData(DENS_VAR,i,j,k) * hx_Avogadro / relA
           ...
           if (eint1 > 0.0 .AND. q1dot .LE. 0.0) then
              ...
              energy1LogRate = 0.0
           else if (eint_zone .NE. 0.0) then
              energy1LogRate = abs(q1dot) / eint_zone
           else
           ...
           if (eint2 > 0.0 .AND. q2dot .LE. 0.0) then
              ...
              energy2LogRate = abs(q2dot) / eint_zone
           else
...
!!$           print*,'#3:q3dot,eint3:',q3dot,eint3
           if (eint3 > 0.0 .AND. q3dot .LE. 0.0) then
              ...
              energy3LogRate = abs(q3dot) / eint_zone
           else if (eint3 == 0.0 .AND. q3dot > 0.0) then
