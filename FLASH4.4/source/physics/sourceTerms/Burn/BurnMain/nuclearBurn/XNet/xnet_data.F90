...
Module reac_rate_data
  ...
  Real(dp), Allocatable :: dr1dt9(:), dr2dt9(:), dr3dt9(:) ! dYdot/dT9 for each reaction in extended arrays
...
! If ( nnnu > 0 ) Call read_nnu_data(nnnu,data_dir)                                             !NNU
  ...
  If ( iheat > 0 ) Then
    ...
    Allocate (dr1dt9(nan(1)))
    Allocate (dr3dt9(nan(3)))
    Allocate (dr2dt9(nan(2)))
  EndIf
  !$omp end parallel
