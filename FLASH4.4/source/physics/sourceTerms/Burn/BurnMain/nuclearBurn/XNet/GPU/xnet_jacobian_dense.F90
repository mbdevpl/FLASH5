...
Module jacobian_data
  ...
  !-------------------------------------------------------------------------------------------------
  Use, Intrinsic :: iso_c_binding, Only: C_DOUBLE, C_INT, C_INTPTR_T, C_NULL_PTR, C_PTR, C_SIZE_T
  ...
  Type(C_PTR) :: dindx, dinfo
  !$omp threadprivate(djac,drhs,dindx,dinfo)

  ! Device pointers to array of device pointers
  Type(C_PTR) :: djac_array, drhs_array
  !$omp threadprivate(djac_array,drhs_array)
...
Subroutine read_jacobian_data(data_dir)
  ...
  Use controls, Only: iheat, nzbatchmx, lun_stdout, myid
  Use cublasf, Only: cublasCreate_v2, cublasSetStream_v2, cublasSetVector, CUBLAS_STATUS_SUCCESS
  Use cudaf, Only: cudaDeviceProp, cudaGetDeviceCount, cudaGetDeviceProperties, cudaHostAlloc, &
    & cudaHostAllocDefault, cudaMalloc, cudaSetDevice, cudaStreamCreateWithFlags, &
    & cudaStreamNonBlocking, cudaStreamDefault, cudaSuccess, cudaEventCreateWithFlags, cudaEventDefault
  Use dbatchedf, Only: doff1d
  Use jacobian_data
  ...
  ! Leading dimension for arrays on device (divisible by 32 for MAGMA)
  maxm = ((msize+31)/32)*32
  maxn = maxm
  ldda = msize
...
! istat = cudaStreamCreateWithFlags(stream, cudaStreamDefault)
  ...
  Allocate (djac(nzbatchmx))
  Allocate (drhs(nzbatchmx))
  istat = cudaMalloc(djac(1), ldda*msize*nzbatchmx*sizeof_double)
  istat = cudaMalloc(drhs(1), ldda*nzbatchmx*sizeof_double)
  istat = cudaMalloc(dindx, msize*nzbatchmx*sizeof_int)
  istat = cudaMalloc(dinfo, nzbatchmx*sizeof_int)

  ! Allocate GPU memory for batched GPU operations
  ...
  do i = 1, nzbatchmx
    Call doff1d(djac(i), djac(1), ldda*msize, i)
    Call doff1d(drhs(i), drhs(1), ldda, i)
  end do
  istat = cublasSetVector(nzbatchmx, sizeof_cptr, c_loc(djac(1)), 1, djac_array, 1)
  istat = cublasSetVector(nzbatchmx, sizeof_cptr, c_loc(drhs(1)), 1, drhs_array, 1)
...
Subroutine jacobian_finalize
  ...
  Use cudaf, Only: cudaFree, cudaFreeHost, cudaStreamDestroy
  Use jacobian_data
...
! istat = cudaFreeHost(hinfo)
  ...
  istat = cudaFree(djac(1))
  istat = cudaFree(drhs(1))
  Deallocate (djac)
  Deallocate (drhs)
...
Subroutine jacobian_solve(kstep,yrhs,dy,t9rhs,dt9,mask_in)
  ...
  istat = cublasSetMatrixAsync(msize, msize*nzbatchmx, sizeof_double, hjac, msize, djac(1), ldda, stream)
  istat = cublasSetVectorAsync(msize*nzbatchmx, sizeof_double, hrhs, 1, drhs(1), 1, stream)

  ! Solve the linear system
  istat = cublasDgetrfBatched(handle, msize, djac_array, ldda, dindx, dinfo, nzbatchmx)
  istat = cublasDgetrsBatched(handle, 0, msize, 1, djac_array, ldda, dindx, drhs_array, ldda, hinfo, nzbatchmx)

  ! Copy the solution back to the CPU
  istat = cublasGetVectorAsync(msize*nzbatchmx, sizeof_double, drhs(1), 1, hrhs, 1, stream)
...
Subroutine jacobian_bksub(kstep,yrhs,dy,t9rhs,dt9,mask_in)
  ...
  Use cudaf, Only: cudaStreamSynchronize, cudaEventRecord, cudaEventSynchronize
  Use jacobian_data
  Use nuc_number, Only: ny
  Use timers, Only: xnet_wtime, start_timer, stop_timer, timer_solve
  Use xnet_types, Only: dp
  ...
  istat = cudaStreamSynchronize(stream)
! istat = cudaEventRecord(event, stream)
