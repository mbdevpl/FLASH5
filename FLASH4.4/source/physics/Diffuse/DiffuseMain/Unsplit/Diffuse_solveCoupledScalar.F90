!!****if* source/physics/Diffuse/DiffuseMain/Unsplit/Diffuse_solveCoupledScalar
!!
!!  NAME 
!!
...
!!                            integer(IN), dimension(VARDESC_SIZE), OPTIONAL :: factorDdesc,
!!                            integer(IN), dimension(VARDESC_SIZE) :: factorAdesc)
!!
!!
!!  DESCRIPTION 
!!
...
!!                    NumGroups variables.
!!   factorDdesc       : Represents emission in MGD.
...
subroutine Diffuse_solveCoupledScalar (unkVarsDesc, xtraVarDesc, diffCoeffDesc, absorpCoeffDesc, bcTypes, bcValues, &
     dt, scaleFact, theta, pass, factorDdesc, factorAdesc)

  use Grid_interface, ONLY : Grid_advanceMultiDiffusion
  ...
  integer, dimension(VARDESC_SIZE), OPTIONAL,intent(IN):: factorDdesc
  integer, dimension(VARDESC_SIZE), OPTIONAL, intent(IN):: factorAdesc

  integer :: ifa, ifd

  if (present(factorAdesc)) then
     ifa = factorAdesc(VARDESC_VAR)
  else
     ifa = -1
  end if
  if (present(factorDdesc)) then
     ifd = factorDdesc(VARDESC_VAR)
  else
     ifd = -1
  end if
  call Grid_advanceMultiDiffusion(unkVarsDesc(VARDESC_VAR), &
       -1,  &
       ifa, &
       bcTypes, bcValues, &
       dt, 1.0, scaleFact, theta, .FALSE., &
       absorpCoeffDesc(VARDESC_VAR), &
       ifd, &
       pass)
