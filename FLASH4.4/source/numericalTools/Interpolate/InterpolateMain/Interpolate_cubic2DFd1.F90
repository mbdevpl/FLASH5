!!****if* source/numericalTools/Interpolate/InterpolateMain/Interpolate_cubic2DFd1
!!
!! NAME
!!
...
!! SYNOPSIS
!!
!!  Interpolate_cubic2DFd1 (real, intent (in) :: a (1:16),
!!                          real, intent (in) :: x,
!!                          real, intent (in) :: y)
!!
!! DESCRIPTION
!!
!!  Calculates the function value and all rescaled 1st derivative values for a pair [x,y]
!!  of rescaled [0,1] coordinates and the 16 bicubic expansion coefficients. The bicubic
...
!!
!!           Interpolate_cubic2DFd1 (1) = the function value
!!           Interpolate_cubic2DFd1 (2) = the rescaled d/dx value
!!           Interpolate_cubic2DFd1 (3) = the rescaled d/dy value
!!
...
function Interpolate_cubic2DFd1 (a,x,y)
  ...
  real    :: Interpolate_cubic2DFd1 (1:3)  ! declares the function as an array
...
!
  ...
  if (notRescaled) then
      call Logfile_stamp     (x, ' = rescaled x coordinate [Interpolate_cubic2DFd1]'    )
      call Logfile_stamp     (y, ' = rescaled y coordinate [Interpolate_cubic2DFd1]'    )
      call Driver_abortFlash ('[Interpolate_cubic2DFd1] ERROR: [x,y] pair not rescaled!')
  end if
!
!
...
!                                                                                ! used for   * count
!
  b (1:4) = ((a (4:16:4) * x + a (3:15:4)) * x + a (2:14:4)) * x + a (1:13:4)    ! f,dy           12

  Interpolate_cubic2DFd1 (1) = ((b (4) * y    + b (3)) * y  + b (2)) * y + b (1) ! -> f            3
  Interpolate_cubic2DFd1 (3) =  (b (4) * y1p5 + b (3)) * y2 + b (2)              ! -> dy           2
  
  b (1:4) = (a (4:16:4) * x1p5 + a (3:16:4)) * x2 + a (2:16:4)                   ! dx              8

  Interpolate_cubic2DFd1 (2) = ((b (4) * y + b (3)) * y + b (2)) * y + b (1)     ! -> dx           3
!                                                                                                 --
!                                                                                      total =    28
!
!     ...Ready!
