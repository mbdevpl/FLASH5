...
#include "Multispecies.h"
  ...
  real :: suminv
  real :: abar,zbar,sumY
  real :: vr, newVel, newVelx, newVely
...
!For 'cutting out' the PNS using rigid body method
           ...
           if (radCenter <= sim_r_inner) then
              vr      = sqrt((solnData(VELX_VAR,i,j,k)*solnData(VELX_VAR,i,j,k)) + (solnData(VELY_VAR,i,j,k)*solnData(VELY_VAR,i,j,k)))
              newVel = (radCenter/(2.e10))*vr*1.3 !2.e10 is maximum domain radius
              newVelx = newVel*sin(thtCenter)
              newVely = newVel*cos(thtCenter)
              call Grid_putPointData(block, CENTER, DENS_VAR, EXTERIOR, rigid_axis, 1.e5)
              call Grid_putPointData(block, CENTER, TEMP_VAR, EXTERIOR, rigid_axis, 2.28e9) 
              call Grid_putPointData(block, CENTER, VELX_VAR, EXTERIOR, rigid_axis, newVelx)
              call Grid_putPointData(block, CENTER, VELY_VAR, EXTERIOR, rigid_axis, newVely)
           end if
