...
class Database :
    ...
    def get_line(self, angle, next_window_id=2) :
        ...
        stderr.write(str(self.__num_samples) + "\n")
...
def main() :
    ...
    for current_file_index0 in range(start_index, (end_index+1)) :

        current_file_index = 20*current_file_index0 
        # Open the database
        ...
        for angle_index in range(num_angles) :
            ...
            angle = float(angle_index) * dtheta - half_pi

            # Open the output file

            output_filename = path + "/" + output_subdir + "/" + sim_name + "pol" \
