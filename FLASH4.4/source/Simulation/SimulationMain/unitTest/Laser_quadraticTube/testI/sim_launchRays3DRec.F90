!!****if* source/Simulation/SimulationMain/unitTest/Laser_quadraticTube/testI/sim_launchRays3DRec
!!
!!  NAME 
!!
!!   sim_launchRays3DRec
...
!!  DESCRIPTION
!!
!!   This routine launches 8 rays on a circle onto the xy-, xz- or yz-plane of a quadratic
!!   potential tube and records their (x,y) (x,z) or (y,z) exit points and their exit power.
!!   The analysis of these results is done in a separate routine.
!!
!! ARGUMENTS
!!
!!***

subroutine sim_launchRays3DRec ()
...
!
!        For 3D cases: launch a total of 8 rays located on the circle around the tube's
!                      xy-, xz- or yz-plane midpoint (5cm,5cm) and radius of 3cm.
!                      Detailed (x,y), (x,z) or (y,z) locations of launching are (in time order):
!
!                              1) ---> (8,5)
!                              2) ---> (2,5)
!                              3) ---> (5,8)
...
!              corresponds to the true xy-, xz- or yz-plane domain exit point at
!              z,y or x = domain zmax, ymax or xmax. Only one processor can have this
!              specific xy-, xz- or yz-plane domain exit point.
!
!
  ...
  do timestep = 1,8
     ...
     if (.not.success) then
          call Driver_abortFlash ('[sim_launchRays3DRec] ERROR: Could not extract rays X position')
     end if
     ...
     if (.not.success) then
          call Driver_abortFlash ('[sim_launchRays3DRec] ERROR: Could not extract rays Y position')
     end if
     ...
     if (.not.success) then
          call Driver_abortFlash ('[sim_launchRays3DRec] ERROR: Could not extract rays Z position')
     end if
     ...
     if (.not.success) then
          call Driver_abortFlash ('[sim_launchRays3DRec] ERROR: Could not extract rays power')
     end if
     ...
     select case (sim_lasersOrientation)
       ...
       case ('XY')
         ...
         if (sim_globalMe == MASTER_PE) then
             ...
             if (sim_nFocalRays > sim_nRaysMax) then
                 call Driver_abortFlash ('[sim_launchRays3DRec] ERROR: Cannot store all focal rays')
             end if

             sim_rayXexit (sim_nFocalRays) = recvarray (1)
             sim_rayYexit (sim_nFocalRays) = recvarray (2)
       ...
       case default
         ...
         call Driver_abortFlash ('[sim_launchRays3DRec] ERROR: Lasers orientation bad!')
...
!
  if (sim_globalMe == MASTER_PE) then
      ...
      if (sim_nFocalRays /= 8) then
          call Driver_abortFlash ('[sim_launchRays3DRec] ERROR: Not all rays on master processor')
      end if
...
!
  return
end subroutine sim_launchRays3DRec
