!!****if* source/Simulation/SimulationMain/unitTest/ParticlesRefine/Simulation_initBlock
!!
!! NAME
!!
...
!!  Poisson solver test problem with particles representing
!!  some of the mass. The problem is designed to test particles
!!  based refinement
!!
!! 
!! ARGUMENTS
...
subroutine Simulation_initBlock(blockID)
  ...
  use Simulation_data, ONLY: sim_densityThreshold, sim_smlRho
...
!!$     !print *, 'using ', Npeak, ' density peaks:'
!!$     !print *,
  ...
  do k = blkLimitsGC(LOW,KAXIS),blkLimitsGC(HIGH,KAXIS)    
     do j = blkLimitsGC(LOW,JAXIS),blkLimitsGC(HIGH,JAXIS)
        do i = blkLimitsGC(LOW,IAXIS),blkLimitsGC(HIGH,IAXIS)       
           ...
           if(rho>sim_densityThreshold)rho=sim_densityThreshold
           call Grid_putPointData(blockID, CENTER, DMPS_VAR, EXTERIOR, axis, rho)
           rho=0.0
           call Grid_putPointData(blockID, CENTER, PDEN_VAR, EXTERIOR, axis, rho)
        enddo
     enddo
  enddo
