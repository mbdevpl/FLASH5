...
os.system('cp mpi_amr_*.F90 ../../../../mpi_source/.')
os.system('cp *.h ../../../../headers/.')
os.system('cp test_*.F90 ../../../../Tests/.')

# edit the Makefile.gnu in mpi_source and add the file names which need to be
# compiled in

#first check to see if Makefile has already been modified for matlab
file = open("../../../../mpi_source/Makefile.gnu","r")
modified = 0
while 1:
	line = file.readline()
	if line == 'write_blocks_matlab_r4.c \\\n':
		modified = 1
	if line == '':
		break
file.close()
...
if (modified == 0):
	...
	while 1:
		...
		file2.write(line)
		if line == 'sources := \\\n':
			file2.write("write_blocks_matlab_r4.c \\\n")
			file2.write("write_blocks_matlab_r8.c \\\n")
			file2.write("mpi_amr_plotfile_matlab.F90 \\\n")
		if line == '':
			break
	file.close()
	file2.close()
