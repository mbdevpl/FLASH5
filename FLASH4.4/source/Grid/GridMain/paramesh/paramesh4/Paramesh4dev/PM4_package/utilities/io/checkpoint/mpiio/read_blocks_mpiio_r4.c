...
#ifndef REAL8
extern void get_new_type_r4(int, int, int, int, int*, int, int, int, MPI_Datatype*);
extern void get_udim_mpiio_r4(int, int, int, int, int, int, int, int, int*, int*, int*);

#ifdef UNDERSCORE
void read_blocks_mpiio_r4_ (char* file_name_in, 
#endif
#ifdef DOUBLE_UNDERSCORE
void read_blocks_mpiio_r4__ (char* file_name_in, 
#endif
#ifndef UNDERSCORE 
#ifndef DOUBLE_UNDERSCORE
void read_blocks_mpiio_r4 (char* file_name_in, 
#endif
#endif
			  int*  tot_blocks,
			  int*  lnblocks,
			  ...
			  int iorder[])
{
  ...
  */
  ...
  MPI_File_open(MPI_COMM_WORLD, file_name, MPI_MODE_RDONLY, MPI_INFO_NULL, &file_handle);
  disp = 0;

  /* READ THE USER ATTRIBUTES */

  MPI_File_read_at(file_handle, 0, tot_blocks, 1, MPI_INT, &status);
  disp = disp + (MPI_Offset)sizeof(int);
  
  MPI_File_read_at(file_handle, disp, user_attr_1, 1, MPI_FLOAT, &status);
  disp = disp + (MPI_Offset)sizeof(float);

  MPI_File_read_at(file_handle, disp, user_attr_2, 1, MPI_FLOAT, &status);
  disp = disp + (MPI_Offset)sizeof(float);

  MPI_File_read_at(file_handle, disp, user_attr_3, 1, MPI_FLOAT, &status);
  disp = disp + (MPI_Offset)sizeof(float);

  MPI_File_read_at(file_handle, disp, user_attr_4, 1, MPI_FLOAT, &status);
  disp = disp + (MPI_Offset)sizeof(float);

  MPI_File_read_at(file_handle, disp, user_attr_5, 1, MPI_FLOAT, &status);
  disp = disp + (MPI_Offset)sizeof(float);
  ...
  *lnblocks = 0;
  ...
  if (*lnblocks > 0) {
    ...
    MPI_File_read_at(file_handle, disp, lrefine, *lnblocks, MPI_INT, &status);
    disp_old = disp_old + (MPI_Offset)(*tot_blocks*sizeof(int));
    
    /* nodetype */
    disp = disp_old + (MPI_Offset)(n_to_left*sizeof(int));
    MPI_File_read_at(file_handle, disp, nodetype, *lnblocks, MPI_INT, &status);
    disp_old = disp_old + (MPI_Offset)(*tot_blocks*sizeof(int));

    /* which_child */
    disp = disp_old + (MPI_Offset)(n_to_left*sizeof(int));
    MPI_File_read_at(file_handle, disp, which_child, *lnblocks, MPI_INT, &status);
    disp_old = disp_old + (MPI_Offset)(*tot_blocks*sizeof(int));
    
    /* gid */
    disp = disp_old + (MPI_Offset)(n_to_left*(*ngid)*sizeof(int));
    MPI_File_read_at(file_handle, disp, gid, *lnblocks*(*ngid), MPI_INT, &status);
    disp_old = disp_old + (MPI_Offset)(*tot_blocks*(*ngid)*sizeof(int));

    /* bflags */
    disp = disp_old + (MPI_Offset)(n_to_left*(*mflags)*sizeof(int));
    MPI_File_read_at(file_handle, disp, bflags, *lnblocks*(*mflags), MPI_INT, &status);
    disp_old = disp_old + (MPI_Offset)(*tot_blocks*(*mflags)*sizeof(int));
    
    /* coord */
    disp = disp_old + (MPI_Offset)(n_to_left*(*mdim)*sizeof(float));
    MPI_File_read_at(file_handle, disp, coord, *lnblocks*(*mdim), MPI_FLOAT, &status);
    disp_old = disp_old + (MPI_Offset)(*tot_blocks*(*mdim)*sizeof(float));
    
    /* bnd_box */
    disp = disp_old + (MPI_Offset)(n_to_left*(2 * *mdim)*sizeof(float));
    MPI_File_read_at(file_handle, disp, bnd_box, *lnblocks*(2 * *mdim), MPI_FLOAT, &status);
    disp_old = disp_old + (MPI_Offset)(*tot_blocks*(2 * *mdim)*sizeof(float));
    
    /* work_block */
    disp = disp_old + (MPI_Offset)(n_to_left*sizeof(float));
    MPI_File_read_at(file_handle, disp, work_block, *lnblocks, MPI_FLOAT, &status);
  ...
  for (lb = 0; lb < *lnblocks; lb++) {
	...
	get_udim_mpiio_r4(*nvar, *nx, *ny, *nz, ivar, istart, jstart, kstart, iorder, udim, it);
        ...
        if (checkp_on_cc[ivar] == 1) {
          ...
          MPI_File_read_at(file_handle, disp,
			   &unk[(udim[3] * udim[2] * udim[1] * udim[0] * \
				 lb) +
