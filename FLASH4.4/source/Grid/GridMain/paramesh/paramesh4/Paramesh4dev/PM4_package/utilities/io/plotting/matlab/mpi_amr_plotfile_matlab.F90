...
!!****f* utilities/io/plotting/matlab/amr_plotfile_matlab
!! NAME
!!
!!   amr_plotfile_matlab
!!
!! SYNOPSIS
!!
!!   call amr_plotfile_matlab(file_num)
!!   call amr_plotifle_matlab(file_num, l_with_guardcells,  
!!                            user_attr1, user_attr2, user_attr3, 
!!                            user_attr4, user_attr5)
!!
!!   call amr_plotfile_matlab(integer, optional logical, optional char*80,
...
!!     If true, then guardcells are included in the file.  Otherwise 
...
!! CALLS
!!
!!   write_blocks_matlab_r8 or write_blocks_matlab_r4
!!
!! RETURNS
!!
!!   Does not return anything.  Upon exit a matlab readable file has been written.
!!
!! DESCRIPTION
!! 
!!  Subroutine to produce output which can be viewed and manipulated using the
!!  MATLAB software. This routine runs in parallel using the HDF5 library.
!!  It writes out the tree data structure and data stored in PARAMESH blocks.
!!
!!  The files produced will have names of the form 
!!  'paramesh_matlab_######.hdf5'. where '######' is the file_num argument passed into
!!  this routine.
!!
!!  The routine is not, by default, included in the PARAMESH default setup.  In order to
!!  use it an install script in utilities/io/plotting/matlab must first be run and
...
!#define DEBUG
      ...
      subroutine amr_plotfile_matlab (file_num,  & 
     &                                l_with_guardcells, & 
     &                                user_attr_1, & 
     &                                user_attr_2, & 
     &                                user_attr_3, & 
...
! handle user attributes
      ...
      call write_blocks_matlab_r8(filename, & 
     &                          tot_blocks, & 
     &                          tot_blocks_wr, & 
     &                          max_lnblocks, & 
     &                          lnblocks_wr, & 
     ...
     &                          user_attr_5_value)
      ...
      call write_blocks_matlab_r4(filename, & 
     &                          tot_blocks, & 
     &                          tot_blocks_wr, & 
     &                          max_lnblocks, & 
     &                          lnblocks_wr, & 
