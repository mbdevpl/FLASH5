...
!-----Use statements
      ...
      Use prolong_arrays, Only :                                       & 
           ...
           prol_fc_dbz_n,                                              & 
           prol_fc_clean_divb
      use gr_pmDivpres_mod, only :  prol_fc_divpres
      use gr_pmDivpres_mod, only :  gr_pmDivpresApply
...
!--------marked on their interp_mask_faceI = -200, thet means they will
!--------not be modified by the gen_fun prolongation below.
            Divpres:if (prol_fc_divpres) Then

               call gr_pmDivpresApply(recvfx,recvfy,recvfz, &
