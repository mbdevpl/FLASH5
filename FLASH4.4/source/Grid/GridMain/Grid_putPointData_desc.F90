!!****if* source/Grid/GridMain/Grid_putPointData_desc
!!
!! NAME
!!  Grid_putPointData_desc
!!
!! SYNOPSIS
!!
!!  Grid_putPointData_desc(integer(IN) :: blockID,
...
!!  
!!             call Grid_putPointData_desc(blockID, CENTER, DENS_VAR, EXTERIOR, &
!!                               position, dataBlock)
!!  
!!          end do
!!
...
!!          do blockID = 1, localNumBlocks
!!  
!!             call Grid_putPointData_desc(blockID, CENTER, PRES_VAR, INTERIOR, &
!!                               position, dataBlock)
!!  
!!          end do
!!
...
subroutine Grid_putPointData_desc(blockid, gridDataStruct, structIndex, beginCount, &
     position, datablock)
...
#undef Grid_releaseBlkPtr
  ...
  if((beginCount /= INTERIOR) .and. (beginCount /= EXTERIOR)) then
     print *, "Grid_putPointData_desc: beginCount set to improper value"
  ...
  if(beginCount == EXTERIOR) then
     ...
     if (position(1) > imax) then
        call Driver_abortFlash("Grid_putPointData_desc position(1) index larger than block")
     end if

     if ((NDIM > 1) .and. (position(2) > jmax)) then
        call Driver_abortFlash("Grid_putPointData_desc position(2) index larger than block")
     end if
    
     if ((NDIM > 2) .and. (position(3) > kmax)) then
        call Driver_abortFlash("Grid_putPointData_desc position(3) index larger than block")
     end if
    
     if (position(1) < 1) then
        print*,'position is ',position(1)
        call Driver_abortFlash("Grid_putPointData_desc position(1) index smaller than 1")
     end if

     if ((NDIM > 1) .and. (position(2) < 1)) then
        call Driver_abortFlash("Grid_putPointData_desc position(2) index smaller than 1")
     end if
    
     if ((NDIM > 2) .and. (position(3) < 1)) then
        call Driver_abortFlash("Grid_putPointData_desc position(3) index smaller than 1")
     end if
