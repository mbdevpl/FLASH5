...
contains
    ...
    subroutine faceAreas(this, axis, region, areas)
      ...
      case (CARTESIAN)
         associate(dx => gr_delta(IAXIS, this%level), &
                   dy => gr_delta(JAXIS, this%level), &
                   dz => gr_delta(KAXIS, this%level))
#if   NDIM == 1
            area = 1.0
#elif NDIM == 2
            if      (axis == IAXIS) then
      ...
      case (CYLINDRICAL)
         if      (axis == IAXIS) then
            ...
            associate(dz   => gr_delta(JAXIS, this%level), &
                      dPhi => gr_delta(KAXIS, this%level), &
                      r    => ABS(faceCoords(:)))
            ...
            associate(dPhi => gr_delta(KAXIS, this%level), &
                      r    => ABS(faceCoords(:)))
               do       k = 1, hi(KAXIS) - lo(KAXIS) + 1
            ...
            associate(dr   => gr_delta(IAXIS, this%level), &
                      dz   => gr_delta(JAXIS, this%level))
