!!****if* source/Grid/GridMain/paramesh/paramesh4/AmrexTransition/Grid_fillGuardCells_blkid
!!
!! NAME
!!  Grid_fillGuardCells_blkid
!!
!! SYNOPSIS
!!
!!  call Grid_fillGuardCells_blkid(integer(IN) :: gridDataStruct,
!!                           integer(IN) :: idir,
...
#include "Flash.h"
  ...
  use Grid_data, ONLY :  gr_eosModeNow, gr_meshMe, gr_meshComm, &
       gr_gcellsUpToDate
...
#include "Flash_mpi.h"
  ...
  integer,dimension(:),allocatable :: blkList
  ...
  if (.NOT. gr_enableMaskedGCFill) then
     ...
     if (.NOT. maskWarningDone) then
        call Logfile_stampMessage( 'INFO: Grid_fillGuardCells_blkid is ignoring masking.')
        if (gr_meshMe==MASTER_PE) print*,    'INFO: Grid_fillGuardCells_blkid is ignoring masking.'
        ...
        if (present(doLogMask)) then
           if (doLogMask) then
              ...
              if (present(doEos)) then
                 if (doEos) then
                    call Logfile_stampVarMask(gcell_on_cc, needEos, '[Grid_fillGuardCells_blkid]'//tagext, 'gcSet')
                 else
                    call Logfile_stampVarMask(gcell_on_cc, needEos, '[Grid_fillGuardCells_blkid]'//tagext, 'gcSet[no doEos]')
                 end if
              else
                 call Logfile_stampVarMask(gcell_on_cc, needEos, '[Grid_fillGuardCells_blkid]'//tagext, 'gcSet[nop doEos]')
              end if
           end if
        end if
     end if
  ...
  if (no_permanent_guardcells) then
     ...
     if (present(selectBlockType)) then
        ...
        case(ALL_BLKS)
           call Driver_abortFlash('Grid_fillGuardCells_blkid: unsupported value ALL_BLKS for selectBlockType!')
#ifdef DEBUG_GRID
        case default
           call Driver_abortFlash('Grid_fillGuardCells_blkid: unrecognized value for selectBlockType!')
#endif
        end select
     else
  ...
  if(present(unitReadsMeshDataOnly)) then
     if (unitReadsMeshDataOnly) then
        if (gr_gcellsUpToDate) then
           ...
           !Grid_fillGuardCells_blkid then they will continue to be up to date.
