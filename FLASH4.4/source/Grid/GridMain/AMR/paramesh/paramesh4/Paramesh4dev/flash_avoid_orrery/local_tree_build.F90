...
Subroutine local_tree_build ()
  ...
  Logical, dimension(cart_dims) :: periodic = (/ .true. /)
  ...
  if (use_reduced_orrery) then
     !Create a communicator consisting of only those MPI ranks that
     !have at least 1 block.
     if (lnblocks >= 1) then
        inBlkComm = 1
     else
        inBlkComm = MPI_UNDEFINED
     end if

     call MPI_Comm_split(amr_mpi_meshComm, inBlkComm, mype, &
          blkComm, ierr)
     if (MPI_COMM_NULL == blkComm) then
        !"A process may supply the color value MPI_UNDEFINED, 
        !in which case newcomm returns MPI_COMM_NULL"
        return
     end if
  else
     blkComm = amr_mpi_meshComm
  end if


  !Create a periodic cartesian communicator from blkComm.
  call MPI_Comm_size(blkComm, blkCommDims(1), ierr)
  call MPI_Cart_create(blkComm, cart_dims, blkCommDims, periodic, &
       reorder, blkCartComm, ierr)
  if (use_reduced_orrery) then
     call MPI_Comm_free(blkComm, ierr)
  end if
  ...
  !isrc is the source processor to receive data from and
  !idest is the destination processor to send data to.
  !isrc and idest are the same in each loop iteration.
  !The send tag must be blkCartCommMyPE and not mype because
  !the process IDs may be different in the reduced communicator.
  call MPI_Cart_shift(blkCartComm, 0, 1, isrc, idest, ierr)
  Call MPI_Comm_rank(blkCartComm, blkCartCommMyPE, ierr)
  ...
  Do ipass = 1,2  
     ...
     proc = mype
     !-----Loop through all processors in the reduced communicator
     Do iproc = 0, blkCommDims(1)-1
        ...
        !Note that "proc" changes each loop iteration.  "proc" keeps track of
        !the "home process", i.e. the process that the current data originates
        !from.  It MUST refer to the process ID in amr_mpi_meshComm (and not
        !blkCartComm!) communicator because it is passed as an argument to
        !add_block_to_tree.  On the first iteration, i.e. iproc == 0, we send
        !mype (my process ID in amr_mpi_meshComm) to process idest.
        Call MPI_SENDRECV_REPLACE (proc, 1, MPI_INTEGER,            &
             idest, blkCartCommMyPE,                          &
             isrc,  isrc,                          &
             blkCartComm, status, ierr)
     ...
     End Do  ! End Do iproc = 0, blkCommDims(1)-1
  ...
  Deallocate(neigh1)
  Deallocate(block_id)
  call MPI_Comm_free(blkCartComm, ierr)
