...
#include "constants.h"
  ...
  write(*,*) "[gr_amrextAmrexInit] Starting"
  ...
  maxLevel = gr_lrefineMax - 1
  call pp_amr%add   ("max_level", maxLevel)
...
!!$                                    gr_markRefineDerefine)
  ...
  print*,'gr_amrextAmrexInit: amrex_max_level is',maxLevel
  ...
  !      index translation
  allocate(unk     (0:maxLevel))
  if (nfacevar > 0) then
     allocate(facevarx(0:maxLevel))
     allocate(facevary(0:maxLevel))
     allocate(facevarz(0:maxLevel))
  end if
...
!!$  call amrex_init_from_scratch(T_INIT)
  ...
  if (is_periodic_am(IAXIS) /= is_periodic(IAXIS)) then
    call Driver_abortFlash("[gr_amrextAmrexInit] AMReX does not have correct periodicity in X") 
  end if
#if NDIM >= 2
  if (is_periodic_am(JAXIS) /= is_periodic(JAXIS)) then
    call Driver_abortFlash("[gr_amrextAmrexInit] AMReX does not have correct periodicity in Y") 
  end if
#if NDIM == 3
  if (is_periodic_am(KAXIS) /= is_periodic(KAXIS)) then
    call Driver_abortFlash("[gr_amrextAmrexInit] AMReX does not have correct periodicity in Z") 
  end if
#endif
#endif
