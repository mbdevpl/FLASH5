!!****if* source/Grid/GridMain/Chombo/AMR/Grid_markRefineDerefine
!!
!! NAME
...
#include "Flash.h"
  ...
  !Zero the tag mesh variable for each cell.
  !This has the same purpose as the PARAMESH code:
  !newchild(:) = .FALSE.
  !refine(:)   = .FALSE.
  !derefine(:) = .FALSE.
  !stay(:)     = .FALSE.
  ...
  do i = 1, blkCount
     call Grid_getBlkPtr(blkList(i), solnData, SCRATCH_CTR)
     solnData(TAGC_SCRATCH_CENTER_VAR,:,:,:) = FLASH_FALSE
     call Grid_releaseBlkPtr(blkList(i), solnData, SCRATCH_CTR)
  end do
