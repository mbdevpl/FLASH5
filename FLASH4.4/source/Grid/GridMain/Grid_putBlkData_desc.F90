!!****if* source/Grid/GridMain/Grid_putBlkData_desc
!!
!! NAME
!!  Grid_putBlkData_desc
!!
!! SYNOPSIS
!!
!!  Grid_putBlkData_desc(integer(IN) :: blockID,
...
!!  
!!             call Grid_putBlkData_desc(blockID, CENTER, DENS_VAR, EXTERIOR, &
...
!!          do blockID = 1, localNumBlocks
!!  
!!             call Grid_putBlkData_desc(blockID, CENTER, PRES_VAR, INTERIOR, &
!!                               startingPos, dataBlock, dataSize)
!!  
!!          end do
!!
...
subroutine Grid_putBlkData_desc(blockID, gridDataStruct, structIndex, beginCount, &
     startingPos, datablock, dataSize)
...
#include "Flash.h"
  ...
  if((blockid<1).or.(blockid>MAXBLOCKS)) then
     print*,' Grid_putBlkData_desc : invalide blockid '
  ...
  if((beginCount /= INTERIOR) .and. (beginCount /= EXTERIOR)) then
     print *, "Grid_putBlkData_desc: beginCount set to improper value"
  ...
  if ((dataSize(1) > imax) .or. &
     ...
     print *, "Grid_putBlkData_desc: dataSize(1) (2) or (3) too big"
     print *,"You are requesting more cells than block has in a dimension"
     call Driver_abortFlash("Grid_putBlkData_desc: dataSize(1) (2) or (3) too big")
  end if
  ...
  if ((dataSize(1)  < 1) .or. &
     ...
     print *, "Grid_putBlkData_desc: dataSize(1) (2) or (3) too small"
     print *,"You are requesting more < 1 cell in a dimension of block, 1 is the min"
     call Driver_abortFlash("Grid_putBlkData_desc: dataSize(1) (2) or (3) too small")
  end if
  ...
  if(beginCount == EXTERIOR) then
     ...
     if (startingPos(1) > imax) then
        call Driver_abortFlash("Grid_putBlkData_desc startingPos(1) index larger than block")
     end if

     if ((NDIM > 1) .and. (startingPos(2) > jmax)) then
        call Driver_abortFlash("Grid_putBlkData_desc startingPos(2) index larger than block")
     end if
    
     if ((NDIM > 2) .and. (startingPos(3) > kmax)) then
        call Driver_abortFlash("Grid_putBlkData_desc startingPos(3) index larger than block")
     end if
    
     if (startingPos(1) < 1) then
        call Driver_abortFlash("Grid_putBlkData_desc startingPos(1) index smaller than 1")
     end if

     if ((NDIM > 1) .and. (startingPos(2) < 1)) then
        call Driver_abortFlash("Grid_putBlkData_desc startingPos(2) index smaller than 1")
     end if
    
     if ((NDIM > 2) .and. (startingPos(3) < 1)) then
        call Driver_abortFlash("Grid_putBlkData_desc startingPos(3) index smaller than 1")
     end if
  ...
  if(beginCount == EXTERIOR) then
     ...
     if ((startingPos(IAXIS) + dataSize(1) -1) > imax) then
        print *, "Error: Grid_putBlkData_desc"
        call Driver_abortFlash("Grid_putBlkData_desc indicies too large")
     end if
...
end subroutine Grid_putBlkData_desc
