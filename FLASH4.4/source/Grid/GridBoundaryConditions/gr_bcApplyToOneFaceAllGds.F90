...
!!                     characterizes local block to be operated onv
...
!!$  isFaceVarNormalDir = isFaceVarNormalDir.or.(gridDataStruct==CENTER_FACES)
  ...
  call gr_bcGetRegionsMixedGds(gridDataStruct,axis,nextDir(1),nextDir(2),endpointsCtr,regionSizeCtr,&
       regionDataC,regionDataFN,regionDataFT1,regionDataFT2,tileDesc,idest)
...
!!$  end if
  ...
  call Grid_bcApplyToRegionMixedGds(bcType,gridDataStruct,tileDesc%level,&
          ...
          regionSizeCtr,&
          applied,&
       tileDesc,nextDir(1),nextDir(2),endpointsCtr,rightHanded,idest)
  if(.not.applied) then
