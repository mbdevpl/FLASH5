!!****if* source/Grid/GridSolvers/HYPRE/paramesh/gr_hypreExchangeFacBFcB
!!
!!  NAME 
!!
...
#include "FortranLangFeatures.fh"
  ...
  call Timers_start("gr_hypreExchangeFacBFcB")       
...
!!$        call Grid_getBlkPtr(blockID, solnVec)          
        ...
        anyFlux = 0.0
        call AssoMed(facBptr1,facBptr,iFactorB) ! facBptr1(:,:,:) => facBptr(iFactorB,:,:,:)
        call gr_hypreGetFaceBFcB (IAXIS, blkLimits, blkLimitsGC, facBptr1, anyFlux, iVar=0)
        ...
        call Grid_ascReleaseBlkPtr(blockID,facBptr,FACEX)

#if NDIM >= 2    
        call Grid_ascGetBlkPtr(blockID,facBptr,FACEY)
        anyFlux = 0.0
        call AssoMed(facBptr1,facBptr,iFactorB) ! facBptr1(:,:,:) => facBptr(iFactorB,:,:,:)
        call gr_hypreGetFaceBFcB (JAXIS, blkLimits, blkLimitsGC, facBptr1, anyFlux, iVar=0)
        anyFlux = anyFlux*scaleFlux
        call Grid_getBlkData(blockID, CELL_FACEAREA, JLO_FACE, EXTERIOR,& 
             blkLimitsGC(LOW,:), areaLeft(:,:,:), datasizeGC)
        call Grid_putFluxData(blockID,JAXIS,anyFlux,blkLimitsGC(HIGH,:), &
             pressureSlots=pSlots(1:nFluxVars),areaLeft=areaLeft)
        call Grid_ascReleaseBlkPtr(blockID,facBptr,FACEY)
     
#if NDIM == 3
        call Grid_ascGetBlkPtr(blockID,facBptr,FACEZ)
        anyFlux = 0.0
        call AssoMed(facBptr1,facBptr,iFactorB) ! facBptr1(:,:,:) => facBptr(iFactorB,:,:,:)
        call gr_hypreGetFaceBFcB (KAXIS, blkLimits, blkLimitsGC, facBptr1, anyFlux, iVar=0)
        anyFlux = anyFlux*scaleFlux
        ...
        call Grid_ascReleaseBlkPtr(blockID,facBptr,FACEZ)
#endif     
#endif  
        deallocate (anyFlux)
...
!!$#endif
  ...
  call Timers_stop("gr_hypreExchangeFacBFcB") 
