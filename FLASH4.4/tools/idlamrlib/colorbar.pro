...
; NAME:
;   COLORBAR
;   Alternate version to detect color depth... --by dgies
;
; PURPOSE:
...
;   Color depth mod. by Daniel Gies
;
;
; CATEGORY:
;       Graphics, Widgets.
...
;       21/20/03. Added detection of system color depth.  dgies
;-
;
;###########################################################################
;
...
IF scalablePixels THEN BEGIN
   ...
   if !d.n_colors eq long(16777216) then begin
       tv_colorized, bar, xstart, ystart, XSIZE=xsize, YSIZE=ysize, /Normal
   endif else begin
...
ENDIF ELSE BEGIN
   ...
   CASE  StrUpCase(!D.NAME) OF
        ...
        if !d.n_colors eq long(16777216) then begin
            tv_colorized, bar, xstart, ystart, /Normal
        endif else begin
