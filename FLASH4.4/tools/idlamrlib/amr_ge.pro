...
if isAMR2 then begin
	if (amr1.name ne amr2.name) then amr_new.name = amr1.name + ' ge ' $
		+ amr2.name
	amr_new.componentname = '(' + amr1.componentname + ' ge ' $
		+ amr2.componentname + ')'
...
for n=0, amr1.maxlevel do begin
	...
	for m=0, amr1.levels[n].nfab-1 do begin
		...
		; do consistency checks on amr1 and amr2 in this fab
		   ...
		   ; form the ge
		   datage = *(*amr1.levels[n].fabptr)[m].dataptr ge $
			    *(*amr2.levels[n].fabptr)[m].dataptr
		endif else begin
		   datage = *(*amr1.levels[n].fabptr)[m].dataptr ge amr2
		endelse

		; point to the result
		(*amr_new.levels[n].fabptr)[m].dataptr = ptr_new(datage)
