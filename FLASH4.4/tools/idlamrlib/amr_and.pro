...
; This function does a cell-wise and of two amr objects of
; identical structure and returns a new amr object. If amr2 is a pure
; number rather than an amr object, it is simply anded with every cell
; of amr1.
...
for n=0, amr1.maxlevel do begin
	...
	for m=0, amr1.levels[n].nfab-1 do begin
		...
		; do consistency checks on amr1 and amr2 in this fab
		   ...
		   ; form the AND
		   dataand = *(*amr1.levels[n].fabptr)[m].dataptr and $
			     *(*amr2.levels[n].fabptr)[m].dataptr
		endif else begin
		   dataand = *(*amr1.levels[n].fabptr)[m].dataptr and amr2
		endelse

		; point to the result
		(*amr_new.levels[n].fabptr)[m].dataptr = ptr_new(dataand)
