!!****if* source/Grid/GridMain/paramesh/paramesh4/Grid_getBlkNeighLevels
!!
!! NAME
!!
...
#include "constants.h"
  ...
  use tree, ONLY : surr_blks, lrefine
  ...
  integer :: myRefine, myType

  myRefine = lrefine(blockID)
  myType = surr_blks(3,2,1+K2D,1+K3D,blockID)

  if (myType .NE. LEAF) then

     levels(:,:,:) = 0
     levels(0,0,0) = myRefine
     where (surr_blks(1,:,:,:,blockID) <= PARAMESH_PHYSICAL_BOUNDARY)
        levels(:,:,:) = myRefine
     elsewhere (surr_blks(3,:,:,:,blockID) == LEAF)
        levels(:,:,:) = myRefine
     end where

  else
     
     where (surr_blks(1,:,:,:,blockID) == -1)
        levels(:,:,:) = myRefine - 1
     elsewhere (surr_blks(3,:,:,:,blockID) == 2)
        levels(:,:,:) = myRefine + 1
     elsewhere
        levels(:,:,:) = myRefine
     end where
