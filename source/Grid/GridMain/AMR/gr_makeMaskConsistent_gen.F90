!!****if* source/Grid/GridMain/paramesh/paramesh4/gr_makeMaskConsistent_gen
!!
!! NAME
!!
...
!!  generic version taking gcell_on_cc as arg  October-2017 KW
!!
!!***
...
#include "Flash.h"
  ...
  logical :: convertToConsvdInGCFill
...
#define HAVE_UNK_VARS_FOR_3T
  ...
  if (present(convertToConsvd)) then
     convertToConsvdInGCFill = convertToConsvd
  else
     convertToConsvdInGCFill = .TRUE.
  end if
...
!!$  end select
  ...
  if(dens) then
     if(.not.gcell_on_cc(DENS_VAR)) then
        if (convertToConsvdInGCFill) then
           do iref=1,NUNK_VARS
