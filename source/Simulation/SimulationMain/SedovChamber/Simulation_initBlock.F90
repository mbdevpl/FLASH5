!!****if* source/Simulation/SimulationMain/SedovChamber/Simulation_initBlock
!!
!! NAME
!!
  ...
  do k = blkLimitsGC(LOW,KAXIS), blkLimitsGC(HIGH,KAXIS)
     ...
     do j = blkLimitsGC(LOW, JAXIS), blkLimitsGC(HIGH, JAXIS)
        ...
        do i = blkLimitsGC(LOW,IAXIS), blkLimitsGC(HIGH, IAXIS)
           ...
           phi = atan(yCoord(j)/xCoord(i)) * 180.0 / PI

           ! if( (( r >= 0.9 .and. r <= 1.0) .and. &
           !      (phi <= 17.5 .or. phi >= 22.5) .and. &
           !      (phi <= 42.5 .or. phi >= 47.5) .and. &
           !      (phi <= 67.5 .or. phi >= 72.5)) .or. &
           !      ( (xCoord(i) - 0.461940)**2 + (yCoord(j) - 0.191342)**2 <= 0.001) .or. &
           !      ( (xCoord(i) - 0.353553)**2 + (yCoord(j) - 0.353553)**2 <= 0.001) .or. &
           !      ( (xCoord(i) - 0.191342)**2 + (yCoord(j) - 0.461940)**2 <= 0.001) ) then

           if( (( r >= 0.9 .and. r <= 1.0) .and. &
                (phi <= 17.5 .or. phi >= 22.5) .and. &
                (phi <= 42.5 .or. phi >= 47.5) .and. &
                (phi <= 67.5 .or. phi >= 72.5)) ) then
              call Grid_putPointData(blockId, CENTER, BDRY_VAR, EXTERIOR, axis, 1.0)
              call Grid_putPointData(blockId, CENTER, DENS_VAR, EXTERIOR, axis, 10.0)
           else
              call Grid_putPointData(blockId, CENTER, BDRY_VAR, EXTERIOR, axis, -1.0)
           end if
