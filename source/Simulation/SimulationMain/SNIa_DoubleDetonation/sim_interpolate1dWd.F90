...
#include "constants.h"
  ...
  if ( r_inner > sim_wd_radius ) then
     ...
     if ( volume > sim_wd_volume ) then
        ...
        call interp1d_linear(sim_wd_vol_tab(1:sim_wd_npnts), sim_wd_dens_tab(1:sim_wd_npnts)*sim_wd_temp_tab(1:sim_wd_npnts), &
                             volume, temp)
        do i = 1, sim_wd_nspec
           call interp1d_linear(sim_wd_vol_tab(1:sim_wd_npnts), sim_wd_dens_tab(1:sim_wd_npnts)*sim_wd_spec_tab(1:sim_wd_npnts,i), &
                                volume, x(i))
        end do
        temp = temp / dens
        x(:) = x(:) / dens
