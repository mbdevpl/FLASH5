...
subroutine io_writeData(fileID) 
  ...
  use Grid_interface, ONLY : Grid_getGlobalIndexLimits, &
       Grid_getLeafIterator, Grid_releaseLeafIterator,Grid_getBlkCornerID
...
!!  allocate(unkt(1,NXB,NYB,NZB,1))
 ...
 !get size of block, this could change from block to block so we can't use NXB, NYB, NZB 
  ...
  do while(itor%is_valid())
     ...
     lo(IAXIS) = blkLimitsGC(LOW,IAXIS)+io_iguard
     hi(IAXIS) = blkLimitsGC(HIGH,IAXIS)-io_iguard     
     lo(JAXIS) = blkLimitsGC(LOW,JAXIS)+io_jguard*K2D
     hi(JAXIS) = blkLimitsGC(HIGH,JAXIS)-io_jguard*K2D  
     lo(KAXIS) = blkLimitsGC(LOW,KAXIS)+io_kguard*K3D
     hi(KAXIS) = blkLimitsGC(HIGH,KAXIS)-io_kguard*K3D   
     ...
     do u=1, ubound(io_unklabelsGlobal,1)
        ...
        if(io_doublePrecision) then
           unkBuf(1,1:localnxb,1:localnyb,1:localnzb,1) = &           
                unk(i,lo(IAXIS):hi(IAXIS),lo(JAXIS):hi(JAXIS),lo(KAXIS):hi(KAXIS),1)
     ...
     do i = 1, NFACE_VARS
        faceXBuf(1,1:localnxb+1,1:localnyb,1:localnzb,1) = &
             facevarx(i,lo(IAXIS):hi(IAXIS)+1,lo(JAXIS):hi(JAXIS),lo(KAXIS):hi(KAXIS), 1)
...
!!$
           faceYBuf(1,1:localnxb,1:localnyb+1,1:localnzb,1) = &
                facevary(i,lo(IAXIS):hi(IAXIS),lo(JAXIS):hi(JAXIS)+1,lo(KAXIS):hi(KAXIS),1)
        ...
        if(NDIM > 2) then
           faceZBuf(1,1:localnxb,1:localnyb,1:localnzb+1,1) = &
                facevarz(i,lo(IAXIS):hi(IAXIS),lo(JAXIS):hi(JAXIS),lo(KAXIS):hi(KAXIS),1)
