...
PG_PATH   = /usr/local/pgi7

#HDF5_PATH   = /usr/local/hdf5-1.6.4-icc
#MPI_PATH    = /usr/local/mpich-1.2.6-icc-ifc90
...
NCMPI_PATH = /usr/local/pnetcdf-1.0.1/pg
MPE_PATH   =
...
LINK    =  $(PG_PATH)/bin/pgf90 -Bstatic
...
CONFIG_LIB = -L/usr/lib/gcc-lib/i386-redhat-linux/3.3.4/ -lstdc++ -L/opt/intel_fc_90/lib/  -Bstatic -lblas -L/asc/home/shawn/pg/linux86/6.1/lib -lpgsse2 -lpgsse1
...
#FFLAGS_TEST  = -c -r8 -i4 -O1 -Mnoframe -Mlre    # Somewhat less optimization than -fast
#FFLAGS_TEST  = -c -r8 -i4 -fast -O4 -Munroll=c:13 -Mipa=fast -Minline -Mvect -Mcache_align -Mflushz
...
#TVIEW = /usr/local/toolworks/totalview.7.1.0-0/linux-x86   #OLD VERSION.
CONFIG_LIB = -L/usr/lib/gcc-lib/i386-redhat-linux/3.3.4/ -lstdc++ -L/opt/intel_fc_90/lib/ -Bstatic -lsvml -Bstatic -lblas -L/asc/home/shawn/pg/linux86/6.1/lib -lpgsse2 -lpgsse1 -Bdynamic -L$(TVIEW)/lib -ltvheap -Wl,-rpath=$(TVIEW)/lib
...
FFLAGS_WO_MBOUNDS = $(patsubst -Mbounds,,$(FFLAGS))
# Compile the following file with the same flags as others (which depend on
# whether -opt, -debug, or -test was in effect for setup), except that
# -Mbounds is omitted . This allows compilation ofthese files
# on the compiler version used for testing on zingiber.
#
# Without this workaround, the compiler refuses to compile references like
# solnVec(SPECIES_BEGIN:SPECIES_END) when SPECIES_BEGIN is above NUNK_VARS
# even though SPECIES_END < SPECIES_BEGIN so this is a valid reference to
# an array of size 0.
Conductivity_fullState.o: %.o : %.F90
	$(FCOMP) $(FFLAGS_WO_MBOUNDS) $(F90FLAGS) $(FDEFINES) $<
endif
